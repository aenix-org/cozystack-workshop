...
`kind: Postgres` — another catalog item, just like `Bucket` in the first phase. You are not installing a database engine: you are ordering one. The platform itself brings up the processes, sets up replication, arranges a backup schedule, and wires in monitoring.
