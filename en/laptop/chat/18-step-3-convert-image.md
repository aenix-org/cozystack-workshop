...
📍 **Where:** inside the converter machine you just logged into over the console. Not on your laptop.
...
Run it — always through `sudo`, the script needs root privileges:
```bash
...
**What to do with it:** copy it into that same notepad. In the next step you'll go back to your laptop, open `manifests/03-app-vm.yaml` and paste it into the `url` field — where
