...
Because the config has the addresses `192.168.10.30` and `192.168.10.40` hard-coded into it — the database and the queue, which lived on **two other VMs in vSphere**. We didn't bring them over and never intended to. There's nothing at those addresses here.
...
**What we do next.** We're going to fix not the application but its picture of the world: in place of the hard-coded IPs we'll put the names of the managed services we brought up in step 5.
