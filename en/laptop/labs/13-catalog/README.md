...
| **What you need** | `helm` on the laptop, `kubectl`, tenant access. The `lab` cluster is not needed here |
...
The error is caught on the laptop in half a second. Without the schema it would have gone off
...
📍 **Where:** on the laptop. No cluster is needed for this.
...
📍 **Where:** on the laptop, with tenant access.
...
# client means the whole check runs on the laptop and the cluster never even hears about it.
