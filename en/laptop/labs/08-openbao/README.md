...
> Plan it as one continuous sitting, with no break in the middle.
...
repository lives on the laptop of a contractor who did an integration last year.
...
hard-coded; who has read it is unknown, because reading a file from Git is recorded nowhere.
...
`kind: OpenBAO` is the catalog item. Mind the case: `OpenBAO`, not `OpenBao`. The cluster
is fussy about case.
...
The tenant's kubeconfig is taken from the dashboard: **Info → Secrets tab →
`kubeconfig-tenant-workshopXX`**. Save it to `~/.kube/workshop`.
...
store with, and you won't have to install it on the laptop. Substitute your own tenant number:
...
⚠️ **Copy both values into a file on the laptop right now** — say into
...
the laptop — the day after tomorrow it's useless. A password in a manifest has no such property.
