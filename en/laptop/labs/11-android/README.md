...
request a guest pass. For now they build it on one developer's laptop. When he's on vacation,
there's no release.
...
The tenant access file (the kubeconfig) is fetched from the dashboard: **Info → the Secrets
tab → `kubeconfig-tenant-workshopXX`**, and saved to `~/.kube/workshop`. That's the same path
