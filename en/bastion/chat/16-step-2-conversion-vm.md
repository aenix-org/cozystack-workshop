...
**Why a separate machine, and not the VM itself.** The tool is called `virt-v2v`, and it drags along a mountain of dependencies and chews through tens of gigabytes. Installing it on the shared bastion that everyone works from is a bad idea: it's small and there's only one of it for all of you. It's easier to bring up a throwaway machine in the cluster right next to the storage, do the job inside it, and shut it down.

As a bonus, this is exactly the approach used for conversion in real migration projects: the converter lives next to the data, not on someone's workstation over a VPN.
