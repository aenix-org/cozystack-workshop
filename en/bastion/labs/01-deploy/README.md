...
Go to this lab's folder — the materials are in `~/workshop`:
...
that will serve our page), the same as would sit in `/etc/nginx/conf.d/` on the bastion.
...
outside. The command below runs a tunnel from the bastion inward.

```bash
# port-forward = a tunnel from the bastion into the cluster, alive as long as the command runs.
...
#   8080:80        the left number is the port on the bastion, the right is the Service port in the cluster
