...
| **What you'll need** | Access to the tenant dashboard, the tenant `~/.kube/config`, `kubectl`, `virtctl` |
...
📍 **Where:** on the bastion (in the bastion terminal), in a separate window. Or right in the browser on your own laptop.
...
If the screen is blank after connecting — press Enter, and the login prompt will appear. To exit the console — `Ctrl+]`. The names of all running instances in your tenant are shown by `kubectl --kubeconfig ~/.kube/config get vminstance -n tenant-workshopXX`.
...
📍 **Where:** on the bastion (in the bastion terminal). Or in the browser on your own laptop.
...
📍 **Where:** on the bastion (in the bastion terminal), in the same window where you worked with `kubectl`.
