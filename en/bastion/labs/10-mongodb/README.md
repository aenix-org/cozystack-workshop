...
file needs to be the tenant one too. On the bastion it's already set up — `~/.kube/config` (the
same path in every lab, token-based access, no browser opens).
...
#   --kubeconfig ~/.kube/config  which access file to use. Without it kubectl
...
MongoDB command shell — and you won't have to install it on the bastion.
