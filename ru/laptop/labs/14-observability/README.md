...
| **Что понадобится** | Кластер из лабы 0, приложение из лабы 1, выполненная лаба 3 (нагрузка и HPA), доступ в дашборд тенанта; приложение `Monitoring` в тенанте (без него метрики некуда складывать и шаги 2–5 не работают) |
...
метрики некуда складывать: у `vmagent` нет достижимого хранилища (адрес отправки не резолвится, `NXDOMAIN`), поэтому исторические метрики не копятся, а шаги 2–5 не работают. Надёжный путь — развернуть `Monitoring`
