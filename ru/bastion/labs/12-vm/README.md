...
>
> 🛠 **Ведущему.** Эти `Service` и `Ingress` заводятся на все тенанты одним прогоном из репозитория, а не мышкой по одному: `instructor/12-vm/provision-spravochnik.sh --context <ctx> --from 1 --to 70` (идемпотентно, безопасно повторять перед каждым потоком). Манифест и инструкция — в `instructor/12-vm/`.
