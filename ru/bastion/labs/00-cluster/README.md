...
> **Со своего бастиона или ноутбука.** На выданной ведущим виртуалке всё уже стоит — этот блок можно пропустить. Если проходите лабы со своей машины, поставьте заранее: `kubectl`, `helm` (v3+), `virtctl` (для лаб с виртуалками, напр. 12), `git`, `python3`, `curl` — и получите свой тенантный `kubeconfig` (в дашборде → скачать доступ). Без `helm` не пройдёт лаба 13, без `virtctl` не откроется консоль в лабе 12.
