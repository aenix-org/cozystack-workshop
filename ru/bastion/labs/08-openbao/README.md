...
Второе требование аудитора — «покажите, кто читал пароль». За это отвечает журнал аудита.

> **Важное про версию.** Начиная с OpenBao 2.5, включить аудит через API командой
> `bao audit enable ... file_path=stdout` **нельзя** — придёт ответ
> `cannot enable audit device via API; use declarative, config-based audit device management instead`.
> Это сознательное ужесточение после уязвимости CVE-2025-54997 (устройство аудита типа
> `file` умело писать в произвольный файл на узле). Теперь аудит задаётся **только
> декларативно** — стензой в конфиге сервера OpenBao, которая применяется при старте и по SIGHUP:
>
> ```hcl
> audit "file" {
...
> ```
>
> В текущем воркшоп-приложении OpenBao эта опция конфига **пока не проброшена**, поэтому
> практически включить аудит в лабе сейчас нельзя — шаг оставлен ознакомительным, и
> проверка (`check.sh`) его не требует. Как только платформа начнёт отдавать audit-конфиг,
> шаг снова станет рабочим.

<details>
<summary><b>Что попадает в аудит-лог и чем он отличается от обычного лога</b></summary>

Когда аудит включён, OpenBao пишет запись **на каждый запрос к API**: кто спросил (какой
...
⚠️ **Даже когда аудит включат, прочитать журнал в лабе вам не дадут — и это надо сказать прямо.** Он
направлен в стандартный вывод пода OpenBao, а логи подов в тенанте ваша роль читать не может —
