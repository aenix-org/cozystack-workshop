#!/usr/bin/env bash
# 실습 검증 스크립트를 위한 공용 라이브러리.
# 다음과 같이 포함합니다:  . "$(dirname "$0")/../../check/lib.sh"
#
# `set -e`는 의도적으로 사용하지 않습니다: 스크립트는 첫 실패에서 멈추지 않고 모든
# 검사를 실행하여 전체 그림을 보여줘야 합니다. 독자는 바로 막혔을 때 이 스크립트를
# 실행합니다 — 중간에 끊는 것은 답의 절반을 숨기는 셈입니다.
...
# 색상은 출력이 터미널로 갈 때만 사용합니다: 파일과 CI에서는 이스케이프 시퀀스가
# 깨진 문자로 읽힙니다.
if [ -t 1 ]; then
...
# --- 기계 판독용 결과 --------------------------------------------------------
# result-<lab>.json은 사람용 보고서와 함께 생성되며 검사 식별자와 그 결과만 담습니다.
# 문구, 명령 출력, 증거는 여기에 들어가지 않습니다: markdown 보고서에는 컨테이너
# 로그의 꼬리, 외부 로드밸런서 주소, 노드 주소, 그리고 사용자 이름과 함께 접근
# 파일 경로가 쌓입니다. 그것을 정규식으로 걸러내는 것은 신뢰할 수 없습니다 —
# 아예 생성하지 않는 것이 신뢰할 수 있습니다.
#
# 식별자는 자동으로 파생됩니다: 실습 내 검사의 순번에 문구의 짧은 해시를 더한 값입니다.
# 순번은 안정성을 주고, 해시는 텍스트의 눈에 띄지 않는 수정을 잡아냅니다 —
# 문구가 바뀌었다면 서비스는 그것을 알아채고 같은 검사로 조용히 인정하지 않습니다.
_checks=()
_seq=0
...
# evidence "제목" "값" — 아티팩트에 들어가며 터미널에는 출력되지 않습니다.
# 증거는 보고서를 누군가에게 보여줬을 때 실제로 의미를 갖도록 하기 위해 필요합니다.
evidence() {
  _evidence+=("### $1")
  _evidence+=('```')
  _evidence+=("$2")
...
# 조기 종료도 보고서는 반드시 남겨야 합니다: README는 "커뮤니티에 오실 때 스크립트
# 보고서를 첨부하라"고 안내하는데, 예전에는 클러스터에 접근할 수 없을 때 첨부할 것이
# 없었습니다 — 즉 정작 보고서가 필요한 바로 그 경우에 보고서가 없었습니다.
need_kubeconfig() {
  if [ -z "${KUBECONFIG:-}" ]; then
    fail "KUBECONFIG 변수가 설정되지 않았습니다" \
         "먼저: export KUBECONFIG=~/lab.kubeconfig (새 터미널 창을 열 때마다)"
    finish; exit 1
  fi
  if ! kubectl version -o json >/dev/null 2>&1; then
    fail "KUBECONFIG=${KUBECONFIG} 로 클러스터가 응답하지 않습니다" \
         "kubectl get nodes 가 응답 없이 멈춰 있다면 — 클러스터 제어 서버가 올라오지 않은 것입니다; 대시보드에서 Kubernetes 애플리케이션 상태와 테넌트 이벤트의 쿼터 부족(exceeded quota)을 확인하세요"
...
need_tenant() {
  if [ -z "${COZY_TENANT:-}" ]; then
    ...
    printf '         %s예를 들어: export COZY_TENANT=workshop07%s\n' "$_C_DIM" "$_C_OFF"
    exit 1
  fi
}

# GNU 확장 없이 시간 처리: macOS의 BSD date는 `-d`를 이해하지 못합니다.
_now() { date -u '+%Y-%m-%d %H:%M:%S UTC'; }
_stamp() { date -u '+%Y%m%d-%H%M%S'; }

# 기계 판독용 결과가 저장되는 위치. 의도적으로 저장소 밖에 둡니다: 클론 안에 두면
# 첫 `git pull`이나 브랜치 전환에 지워지지만, 이 결과는 몇 주에 걸쳐 수집됩니다.
...
finish() {
...

  {
    ...
    echo "---"
    echo
    echo "이 보고서는 Cozystack 실습의 \`check.sh\` 스크립트로 생성되었습니다."
    echo "매니페스트 적용 여부가 아니라 실제로 동작하는지를 검증했습니다."
  } > "$report"
...
# 특별히 서버 버전. `kubectl version -o json`은 클라이언트와 서버 버전을 모두
# 출력합니다; gitVersion에 대한 순진한 grep은 처음 발견한 것 — 클라이언트 — 을
# 가져와 보고서가 클러스터 버전에 대해 거짓말을 하기 시작합니다. 여기서 실수하기
# 쉬워서 라이브러리로 분리했습니다.
server_version() {
...
# 사람이 읽을 수 있는 크기: Kubernetes는 allocatable을 어떤 때는 Ki로, 어떤 때는
# 순수 바이트로 보고하며, 보고서의 "3258002390"은 독자에게 아무 의미가 없습니다.
human_bytes() {
...
# 설정한 환경 변수로 전달합니다.
#
# 왜 이렇게 하는가. 파드의 args에 들어가는 모든 것은 `get pods` 권한이 있는 누구에게나
# 보이고, etcd에 저장되며, audit log로 가고, 노드의 `ps`에 드러납니다. 데이터베이스
# 실습은 명령줄의 비밀번호가 나쁜 관행이라고 별도로 설명합니다; 정확히 그것을 하는
# 스크립트로 그것들을 검증한다면 이중 잣대가 됩니다.
#
# 사용법:
#   in_cluster_with_secrets "<image>" "KEY1=val1
#   KEY2=val2" sh -c '$KEY1 을 읽는 명령'
...
EOF
  ...
  # securityContext는 여기서도 필수입니다: 이것이 없으면 `restricted` 프로필의
  # 클러스터에서 파드가 생성되지 않고, 데이터베이스 실습의 검사가 동작하지 않습니다.
  local cmd_json
...
# `restricted` 프로필을 통과하는 securityContext가 포함된 override를 구성합니다.
# 별도로 분리한 이유: 모든 일회용 파드에 동일한 부가 설정이 필요하며,
# 이것이 없으면 검증 스크립트가 엄격한 클러스터에서 동작하지 않습니다.
# 명령 인자는 각각 따로 전달하고, JSON은 python이 조립합니다:
# bash에서 따옴표를 수동으로 이스케이프하다가 이미 깨진 override와 파드의 조용한
# 실패로 이어진 적이 있으며 — 그때 오류는 2>/dev/null로 묻혔습니다.
...
# 클러스터 내부에서 서비스 가용성을 검증하는 곳에서 필요합니다: 노트북에서는
# ClusterIP가 보이지 않습니다. 파드는 어떤 경우든 스스로 정리됩니다.
in_cluster_curl() {
  local url="$1" extra="${2:-}"
  local name="check-$$-$RANDOM"
  # securityContext는 필수입니다: `restricted` 프로필의 클러스터에서 이것이 없는
  # 파드는 생성되지 않으며, 참가자는 실습을 아예 검증할 수 없게 됩니다.
  kubectl run "$name" --rm -i --restart=Never --quiet \
    ...
    2>/dev/null
  local rc=$?
  # `--rm`은 클라이언트가 연결되어 있는 동안에만 파드를 삭제합니다: 연결 끊김,
  # 타임아웃, Ctrl+C는 파드를 매달린 채로 남깁니다. 명시적 삭제 — 스크립트가
  # 클러스터를 어지럽히지 않도록 하기 위함입니다.
  kubectl delete pod "$name" --ignore-not-found --wait=false >/dev/null 2>&1
  return $rc
}

# 여러 번의 요청 응답을 한 줄에 하나씩 연달아 수집합니다.
#
# 서비스 뒤에 여러 복제본이 있을 때 단일 요청은 복불복입니다: 같은 레이블을 가진
# 엉뚱한 파드가 밸런싱에 끼어들지만, 한 번의 표본은 그것을 놓칠 수 있고, 검사는
# 바꿔치기된 콘텐츠 위에서 기쁘게 초록불이 됩니다. 확인됨: 스무 번의 요청 중 여덟
# 번이 사칭 파드로 갔고, 그동안 검사는 네 번 연속 "합격"이라고 말했습니다.
