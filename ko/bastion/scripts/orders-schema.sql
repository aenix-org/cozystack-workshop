...
-- 없습니다. 먼저 한 번, superuser(secret postgres-db-superuser)로:
--     GRANT CREATE,USAGE ON SCHEMA public TO orders;
-- 그런 다음에만 orders 역할로 이 파일을 적용하세요. 테이블이 없으면 애플리케이션은
-- POST /api/orders에 500을 반환합니다 (health는 200을 유지 — PG 연결만 확인할 뿐
-- 테이블 존재 여부는 확인하지 않습니다).
--
