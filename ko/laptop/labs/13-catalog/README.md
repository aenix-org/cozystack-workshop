...
| **필요한 것** | 노트북 위의 `helm`, `kubectl`, 테넌트 접근 권한. 여기서는 `lab` 클러스터가 필요 없습니다 |
...
오류는 노트북에서 0.5초 만에 잡힙니다. 스키마가 없었다면 이것은 클러스터로 넘어가, 절대 생성되지 않는
...
📍 **어디에서:** 노트북에서. 이 작업에는 클러스터가 필요 없습니다.
...
# client는 모든 검사가 노트북에서 이뤄지고 클러스터는 그에 대해 알지도 못한다는 뜻입니다.
...
📍 **어디에서:** 노트북에서, `kubectl`로 작업하던 바로 그 터미널 창에서.
