#!/bin/bash
# Cambiar la aplicación migrada de IPs hardcodeadas legacy a endpoints DNS gestionados.
# Ejecutar en la app-VM como root DESPUÉS de netfix-dhcp.sh + reinicio (de lo contrario DNS no resuelve).
set -e
CONF=/etc/orders/application.properties

# =========================================================================
# PEGUE LOS NOMBRES DE SUS servicios gestionados.
#   Formato de Service DNS en su tenant (namespace = tenant-<su-login>):
...
# Reemplace solo tenant-workshopXX por su namespace. Si nombró Postgres/Kafka de otra forma —
# cambie db/kafka por sus nombres.
