#!/usr/bin/env bash
# Verificación del lab 2: autorreparación.
#
# Verificamos no «los comandos fueron escritos» sino el estado del clúster tras el lab: la
# aplicación vuelve a atender solicitudes a través del Service, devuelve el nombre de su réplica,
# y ese nombre pertenece a un pod realmente en ejecución. Además buscamos rastros de que las
# réplicas fueron recreadas.
#
# El script no elimina ni crea nada, salvo un pod de un solo uso para comprobar la disponibilidad
# del servicio desde dentro del clúster — se elimina a sí mismo.
...
# BSD date en macOS y GNU date en Linux interpretan las fechas de forma distinta, mientras que
# python está en todas partes donde funciona lib.sh.
_epoch() {
...
if [ -z "$DEP_TS" ]; then
  fail "la aplicación ${APP} no está en el clúster" \
       "al final del lab había que devolverla: kubectl apply -f ../01-deploy/rickroll.yaml"
...
# --- cadena Deployment -> ReplicaSet -> Pod --------------------------------
# El sentido del lab es que la réplica la devuelve el ReplicaSet, no «el clúster en general».
# Si el propietario del pod no resulta ser un ReplicaSet, significa que el participante levantó
# el pod a mano, y no verá la autorreparación.
# Contamos los pods por nombre, en lugar de recopilar los tipos únicos de propietarios: para un pod
# sin ownerReferences jsonpath devuelve una cadena vacía, `sort -u` la colapsa en un elemento
# invisible, y `*ReplicaSet*` coincide mientras al menos un pod esté gestionado por un ReplicaSet.
# Por eso un pod ajeno, levantado a mano, pasaba la verificación sin ser detectado.
...
case "${PODS_TOTAL}:${PODS_BY_RS}" in
  0:*)
    fail "no hay ni un solo pod con la etiqueta app=${APP}" \
         "devuelva la aplicación: kubectl apply -f ../01-deploy/rickroll.yaml"
    ;;
  *:0)
    fail "ningún pod de ${APP} está gestionado por un ReplicaSet — no habrá autorreparación" \
         "parece que el pod se levantó a mano (kubectl run). Elimínelo y aplique ../01-deploy/rickroll.yaml"
    ;;
  *)
    if [ "$PODS_TOTAL" -ne "$PODS_BY_RS" ]; then
      fail "la etiqueta app=${APP} la llevan pods ajenos: ${PODS_BY_RS} de ${PODS_TOTAL} están gestionados por ReplicaSet" \
...
# --- rastros de la recreación de réplicas ----------------------------------
# El clúster no guarda pruebas directas de que «el pod fue matado». Hay dos indirectas, y ambas
# son suficientes: el pod es notablemente más joven que su Deployment, y en los eventos del
# ReplicaSet hay más de una creación.
...
if [ -n "$DEP_E" ] && [ -n "$POD_E" ]; then
  DELTA=$(( POD_E - DEP_E ))
  if [ "$DELTA" -ge 45 ]; then
    ok "la réplica es ${DELTA} s más joven que la aplicación — o sea que la anterior se eliminó y esta se creó en su lugar"
  else
    warn "la réplica tiene casi la misma edad que la aplicación (diferencia ${DELTA} s)" \
         "si restauró la aplicación completa justo al final — es normal; de lo contrario el paso de eliminar el pod no se hizo"
  fi
...
# Los eventos viven cerca de una hora, así que su ausencia no es un fallo sino una observación.
...
if [ "$CREATES" -ge 2 ]; then
  ok "en los eventos del clúster hay ${CREATES} creaciones de réplica — la autorreparación realmente se activó"
  ...
  warn "en los eventos del clúster se ve la creación de réplica solo ${CREATES} vez(veces)" \
       "los eventos se guardan cerca de una hora y pudieron caducar"
fi

# Ninguno de los dos indicios por separado es bloqueante: los eventos viven cerca de una hora,
# y la edad coincide para quien legítimamente restauró la aplicación completa al final del lab.
# Pero si NO se cumple ninguno — la réplica no se eliminó en absoluto, y el lab no está hecho. Sin
# esta combinación el script imprimía «LAB APROBADO» justo después del lab 1, sin esperar ni una eliminación.
if [ "${DELTA:-0}" -lt 45 ] && [ "$CREATES" -lt 2 ]; then
  fail "no se encontraron rastros de autorreparación: la réplica no se eliminó" \
       "elimine la réplica: kubectl delete pod -l app=${APP} — y ejecute la verificación dentro de una hora, mientras los eventos siguen vivos"
fi

# --- el servicio realmente atiende -----------------------------------------
# La verificación principal de fondo: no «el objeto existe» sino «a través del Service llega una
# página y en ella está el nombre de una réplica viva».
...
if [ -z "$BODY" ]; then
  ...
  fail "la página se sirve, pero el nombre de la réplica no se insertó en ella" \
...
if [ "$WANT" = "1" ]; then
  ok "la cantidad de réplicas se devolvió a una — el lab 3 empezará desde cero"
else
  warn "ahora hay solicitadas réplicas: ${WANT}" \
