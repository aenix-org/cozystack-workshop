...
| **Qué necesitarás** | El clúster de la Lab 0, `kubectl`, `~/lab.kubeconfig`, acceso al panel del tenant |
...
Es la primera vez en todas las labs que una carga de trabajo **termina**. Todo lo que hemos
...
Es la distinción clave de la lab, y conviene fijarla antes de aplicar nada.
...
| `check.sh` | Una verificación de que la compilación tuvo éxito y el APK aterrizó en el almacenamiento | lo ejecutas al final de la lab |
...
clúster `lab` de la Lab 0 lo pediste dentro de él, y el bucket lo pedirás ahí también.
...
El archivo de acceso al tenant en este bastion ya está configurado — `~/.kube/config`, la misma
ruta que en todas las labs (acceso basado en token, no se abre ningún navegador).
...
Nos arreglaremos con uno para ahorrar tiempo de la lab, pero conviene saberlo.
...
al kubeconfig de tu clúster de la Lab 0 también.
...
```bash
# De aquí al final de la lab trabajamos con el clúster lab, no con el tenant
...
Todo lo demás en estas labs es texto que puede ir a Git. Un secret no. El objeto `Secret` dentro
...
tiempo en `Pending` y la recreación del nodo un poco más adelante en la lab consumen el mismo
...
Compáralo con la app de la primera lab: `20m` de CPU y `32Mi` de memoria. Una diferencia de
...
segunda notablemente más rápida. A propósito no lo hacemos así en la lab, para no introducir una
...
📍 **Dónde:** en el bastion, en el clúster `lab`.
...
>
> ¿Qué exactamente no cuadró? Recuerda qué nodo pediste en la Lab 0 y cuánta memoria solicitó
> el Job.

<details>
<summary><b>La respuesta, y una lección más amplia que este error</b></summary>

En la Lab 0 tomamos el nodo `u1.medium` — un núcleo y cuatro gigabytes. El Job pide `requests:
...
monitorización, más la app de la primera lab.
...
notablemente más rápido, y devolverás los recursos de más justo después de la lab. Si no lo
...
del nodo desaparece. Para nuestras labs esto es indoloro — los datos viven en servicios
...
describe al inicio de la Lab 0. Puedes completar la lab sin WSL, pero no habrá informe de
artefacto.
