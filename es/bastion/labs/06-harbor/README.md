...
| **Qué vas a necesitar** | El clúster del Lab 0, `kubectl`, `docker` (o `podman`) en el bastion, acceso al panel |
...
|---|---|---|
| `~/.kube/config` | El clúster de gestión de Cozystack, tu tenant | mirar los servicios gestionados: Harbor, bases de datos, colas |
...
📍 **Dónde:** en el panel, luego en una terminal en el bastion.
...
credenciales en tu bastion, en el archivo `~/.docker/config.json`. Después de eso `docker push` y
...
⚠️ **Este `docker login` le enseñó a tu bastion a iniciar sesión en el registro, y solo a él.** No hizo
...
coloca el resultado en el almacén de imágenes de tu bastion. El nombre con el que se guarda el resultado lo
...
#                           y el bastion puede ser ARM — entonces sin el flag obtienes lo que no es
...
⚠️ **`--platform linux/amd64` no es decoración.** Si tienes un Mac con Apple Silicon (M1–M4) o un bastion
...
📍 **Dónde:** en el bastion, el clúster `lab`.
...
Tu `docker login` escribió las credenciales en el archivo `~/.docker/config.json` en **tu bastion**. El nodo
del clúster no sabe nada de ese archivo ni puede: entre él y tu bastion no hay absolutamente nada en común,
...
# que en el archivo ~/.docker/config.json de tu bastion.
...
La aplicación no está expuesta al exterior, pero necesitas verla. `port-forward` cava un túnel desde el
bastion hacia el clúster: mientras el comando se ejecuta, una petición a `localhost:8080` va al servicio
...
#   8080:80 — el número de la izquierda es el puerto en tu bastion, el de la derecha el puerto del servicio en el clúster
...
# Puedes dejarla sin definir: entonces el script busca ~/.kube/config, y al no encontrarlo — omite
...
- Distinguir un `docker login` en el bastion del acceso que necesita el clúster
