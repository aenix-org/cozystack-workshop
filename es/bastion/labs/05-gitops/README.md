...
mkdir -p apps
# Copia ambos archivos de los materiales del taller (están en ~/workshop) a tu propio
# repositorio; `*.yaml` tomará ambos archivos de una vez.
...
# El commit por ahora vive solo en tu bastion, todavía no está en GitHub.
...
Primero, la herramienta de línea de comandos `flux`. Vive en tu bastion, no en el clúster: la usarás para instalar los servicios y luego para preguntarles por su estado.
...
📍 **Dónde:** en el bastion, en la carpeta del repositorio.
