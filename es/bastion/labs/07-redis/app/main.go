...
func envInt(key string, fallback int) int {
	if v := os.Getenv(key); v != "" {
		...
		log.Printf("el valor %s=%q no es un número, uso %d", key, v, fallback)
	}
	return fallback
}
...
func (r *redisClient) readReplyLocked() (string, bool, error) {
	...
	default:
		return "", false, fmt.Errorf("redis: respuesta no reconocida %q", line)
	}
}
...
func main() {
	...
	case "api":
		setupAPI(mux, pod)
	default:
		log.Fatalf("MODE=%q desconocido, los valores permitidos son hr y api", mode)
	}
...
func setupHR(mux *http.ServeMux, pod string) {
	...
	if err != nil {
		log.Printf("no se pudo analizar HR_DELAY=%q, uso 800ms", os.Getenv("HR_DELAY"))
...
func setupAPI(mux *http.ServeMux, pod string) {
	...
	if addr := os.Getenv("REDIS_ADDR"); addr != "" {
		...
		log.Printf("caché activada: %s, tiempo de vida del registro %d s", addr, ttl)
	} else {
		log.Printf("caché desactivada: REDIS_ADDR no está definido, cada petición irá al directorio")
	}
		...
		if cache != nil {
			...
			case found:
				if json.Unmarshal([]byte(raw), &emp) == nil {
					...
					log.Printf("la caché contiene basura bajo la clave %s, voy al directorio", key)
				}
			}
		}
