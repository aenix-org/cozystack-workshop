...
|---|---|---|
| `~/.kube/config` | El clúster de gestión de Cozystack, tu tenant | mirar Redis: dirección, estado |
...
en el bastion, `push` la envía al registro. El nombre de la imagen es el mismo que en el lab
...
⚠️ **`--platform linux/amd64` es obligatorio si tienes un Mac con Apple Silicon o una VM en
...
```bash
# port-forward svc/fortio = un túnel desde el bastion hasta el servicio del generador en el clúster
#   8081:8080 — el número de la izquierda es el puerto en tu bastion, el de la derecha el puerto del servicio en el clúster
...
📍 **Dónde:** en el bastion, el clúster **de gestión**.
...
Ahora — una comprobación de conectividad. 📍 **Dónde:** en el bastion, el clúster **`lab`**.
