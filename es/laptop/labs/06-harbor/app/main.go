...
// La dirección /healthz devuelve un breve «ok»: por ella el clúster entiende que la copia está viva
// y lista para aceptar tráfico. La dirección / devuelve una respuesta pequeña en formato JSON (texto
// del tipo «campo: valor»), donde se enumera qué copia y en qué nodo atendió la
...
// fuente -> imagen -> tu propio registro -> clúster.
//
// No hace falta saber leer este archivo en Go, basta con entender qué ocurre aquí;
// los comentarios de abajo están redactados contando con que ves Go por primera vez.
//
// No hay dependencias externas: solo se usa la biblioteca estándar, que
// viene junto con el compilador. Por eso la compilación no va a internet a por
// bibliotecas, y se puede construir la imagen donde la salida a la red está cerrada, — y precisamente
// con eso empieza toda la lab.
//
...
//
// package main — así se marca en Go un programa que se puede ejecutar (a diferencia
// de una biblioteca). El punto de entrada al ejecutar es la función main al final del archivo.
package main
...
//                   de donde los recoge kubectl logs. No hay archivos de log dentro,
//                   y no hace falta crearlos — esto es lo normal para contenedores
//   net/http      — el propio servidor web
//   os            — leer variables de entorno
//   time          — marca de tiempo en la respuesta y timeout del servidor
import (
	"encoding/json"
	"log"
	"net/http"
...
// son lo que la aplicación aprende del clúster: no los calculamos ni los adivinamos, el clúster
// mismo los pone en variables de entorno (ver passes.yaml, el bloque env y downward API).
//
// El texto entre comillas invertidas a la derecha es el nombre del campo en el JSON final. Sin él el campo iría
// a la respuesta como Namespace, no como namespace; la comprobación en check.sh busca el «pod» en minúscula.
...
func main() {
	// En qué puerto escuchar. El puerto se puede redefinir con la variable PORT, sin recompilar
	...
	// Comprobación de disponibilidad. El clúster llama aquí y no deja pasar tráfico a la copia,
	...
	// La respuesta principal. Armamos esos mismos campos y los entregamos como un único JSON. Los valores se leen
	// en cada petición, por eso la segunda copia del servicio responderá con su propio nombre de pod —
	// por ese nombre en la lab se ve que realmente hay dos copias.
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		enc.SetIndent("", "  ")
		// SetEscapeHTML(false), de lo contrario el cirílico y símbolos como < irían a parar a \uXXXX
	...
	// de cortar la conexión. Los cinco segundos no están por velocidad: sin este timeout
