...
#
# No comprobamos «Harbor fue creado», sino toda la cadena: el registro responde en su API,
# la imagen del manifiesto vive exactamente en él, el clúster tiene credenciales para esa misma dirección,
...
# clúster de gestión de Cozystack, donde en tu tenant vive el servicio gestionado Harbor.
# No se pueden consultar ambos con un solo comando, por eso abajo hay dos formas distintas de llamar a kubectl.
#
...
# Andamiaje común de todos los labs: ok / fail / warn / evidence / finish y comprobaciones del entorno.
. "$(cd "$(dirname "$0")/../../check" && pwd)/lib.sh"

# Sin un archivo de acceso al clúster y sin número de tenant no hay nada que comprobar — salimos de inmediato.
need_kubeconfig
need_tenant

APP="passes-api"
# El namespace del tenant en el clúster de gestión: el nombre se compone del prefijo
# tenant- y tu número, es decir tenant-workshopXX. El número se toma del entorno,
# no hace falta sustituirlo en el texto del script a mano.
...
# resultados esperados, y se maneja abajo en una rama aparte con un consejo claro.
...
# ver aplicaciones. Esto no es culpa del participante ni motivo para tumbar la comprobación, por eso
# aquí es warn — «no miramos», no fail — «hecho mal». Distinguimos a propósito un error del comando
# de una respuesta vacía: una lista vacía significa que Harbor nunca fue creado.
if [ ! -r "$COZY_KUBECONFIG" ]; then
  ...
  if [ -n "$HARBOR_ERR" ]; then
    warn "no se pudieron ver las aplicaciones Harbor en el tenant ${TENANT_NS}" \
         "el rol en el tenant puede no permitir este comando — no es un error del lab; todo lo demás se comprueba abajo"
    ...
    if [ "$HARBOR_READY" = "True" ]; then
      ...
      warn "Harbor «${HARBOR_NAME}» existe, pero no informa que esté listo" \
           "mira su estado en el dashboard; Harbor tarda 5-10 minutos en levantar, y sin almacenamiento de objetos en el tenant no levantará en absoluto"
    fi
...
# --- de dónde saca la imagen la aplicación ----------------------------------
# El sentido del lab es que la imagen vino de tu registro, no de internet. Esto se comprueba
# por el nombre de la imagen en el manifiesto: la primera parte del nombre hasta la barra es la
# dirección del registro. Si no tiene ni punto ni dos puntos, ahí no hay dirección alguna, y el
# clúster iría en silencio a por la imagen a Docker Hub — justo a donde seguridad lo prohibió.
# Capturamos el marcador HARBOR-HOST y los registros públicos conocidos en ramas aparte:
# formalmente la dirección está en su sitio, pero el requisito del lab no se cumple, y el consejo difiere en cada caso.
...
if [ -z "$IMAGE" ]; then
  fail "en el clúster lab no está la aplicación ${APP}" \
       "aplica passes.yaml sustituyendo en él la dirección de tu Harbor"
else
  REGISTRY="${IMAGE%%/*}"
  case "$REGISTRY" in
    *.*|*:*) : ;;              # parece una dirección de registro
    *) REGISTRY="" ;;          # no hay dirección — así que la imagen se saca de Docker Hub
  esac

  if [ -z "$REGISTRY" ]; then
    fail "la imagen ${IMAGE} se saca de un registro público, no del tuyo" \
         "el nombre de la imagen debe empezar por la dirección de tu Harbor"
  elif printf '%s' "$REGISTRY" | grep -qi 'HARBOR-HOST'; then
    fail "en el manifiesto quedó la dirección-marcador HARBOR-HOST" \
         "sustituye la dirección de tu Harbor: sed -i 's|HARBOR-HOST|harbor.tudominio|g' passes.yaml"
  elif printf '%s' "$REGISTRY" | grep -qiE '^(docker\.io|registry-1\.docker\.io|quay\.io|ghcr\.io|gcr\.io|registry\.k8s\.io)$'; then
    fail "la imagen se saca del registro público ${REGISTRY}" \
         "seguridad pidió un registro privado — construye y sube la imagen a tu propio Harbor"
  else
...
# La dirección del manifiesto puede estar escrita correctamente, pero puede que no haya registro en ella: Harbor
# no levanta al instante, y una errata en el dominio tiene exactamente el mismo aspecto. Por eso
# llamamos a su API y esperamos la respuesta «pong» — esto confirma que ahí hay precisamente Harbor,
# y no el sitio de otro ni un stub del balanceador.
if [ -z "$REGISTRY" ]; then
  : # ya se informó arriba
elif ! command -v curl >/dev/null 2>&1; then
  warn "no hay utilidad curl — la disponibilidad del registro no se comprobó" \
       "abre https://${REGISTRY} en el navegador, ahí debe estar la interfaz de Harbor"
else
...
# No basta con que el secreto esté referenciado en el manifiesto — lo que importa es que tenga
# credenciales para exactamente el registro del que se saca la imagen. El error más común del lab parece
# correcto: el secreto está creado, nombrado en el manifiesto, pero la dirección dentro de él no es esa
...
if [ -z "$IMAGE" ]; then
  ...
  for s in $PULL_SECRETS; do
    ...
    # Analizamos la config con python: base64 -d se comporta de forma distinta en macOS y Linux,
    # y la contraseña no debe imprimirse en el informe — tomamos solo la lista de direcciones.
...
try:
  ...
  if [ -n "$SECRET_OK" ]; then
    ...
    fail "ninguno de los secretos indicados (${PULL_SECRETS}) contiene credenciales para ${REGISTRY:-tu registro}" \
         "créalo así: kubectl create secret docker-registry harbor --docker-server=${REGISTRY:-DIRECCION} --docker-username=admin --docker-password=..."
  fi
fi

# --- los pods realmente arrancaron ------------------------------------------
# Manejamos aparte los estados ImagePullBackOff y ErrImagePull: este es justo el fallo
# que el lab muestra a propósito, y para el participante es importante reconocerlo de vista, en lugar
# de recibir un genérico «los pods no funcionan». Imprimimos la causa real como evidencia —
...
if [ "$RUNNING" -ge 1 ]; then
  ok "réplicas de la aplicación en marcha: ${RUNNING}"
  ...
  fail "no hay ninguna réplica de la aplicación en marcha (estados: ${BADSTATE:-no hay pods})" \
       "mira kubectl describe pod -l app=passes-api"
fi

# Una comprobación aparte para el error del lab más difícil de diagnosticar: la imagen se construyó
# para ARM, mientras que los nodos del clúster son x86. Todo parece correcto — la imagen se construyó, se subió
...
if printf '%s' "$LOGS" | grep -q 'exec format error'; then
  fail "la imagen se construyó para otra arquitectura de procesador" \
       "recompílala con el flag: docker build --platform linux/amd64 -t ${IMAGE} app/ y súbela de nuevo"
fi

# --- la aplicación responde de forma sustantiva -----------------------------
# Un pod en marcha aún no significa un servicio que funcione. Vamos dentro del clúster, pedimos la
# aplicación por su nombre interno y leemos de la respuesta el nombre del pod. Si coincide con uno realmente
# en marcha — entonces responde precisamente la aplicación que desplegamos, y no
# otra cosa que ocupó por casualidad esta dirección. Una discrepancia es warn, no fail:
# una réplica pudo recrearse entre dos peticiones, y no es culpa del participante.
if [ -z "$(kget svc "$APP" -o name)" ]; then
  fail "no hay un Service con el nombre ${APP}" \
...
try: print(json.load(sys.stdin).get("pod",""))
  ...
  if [ -z "$SERVED_POD" ]; then
    ...
    ok "el servicio responde con JSON, la respuesta vino del pod realmente en marcha ${SERVED_POD}"
    evidence "Respuesta del servicio" "$BODY"
  else
    warn "el servicio respondió en nombre del pod ${SERVED_POD}, que no está entre los que están en marcha" \
         "lo más probable es que la réplica se recreara entre peticiones — ejecuta la comprobación otra vez"
  fi
fi
