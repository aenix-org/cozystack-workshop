...
# Un agente en marcha que escribe al vacío se ve exactamente igual que uno que
...
if [ "$VMAGENT_RUNNING" -ge 1 ]; then
  ...
  if [ "${SENT:-0}" -gt 0 ]; then
    ...
    fail "el agente está en marcha pero no entregó ni una sola métrica al almacenamiento (0 envíos exitosos)" \
