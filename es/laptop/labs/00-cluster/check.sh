#!/usr/bin/env bash
# Verificación del lab 0: el clúster de formación está levantado y estás conectado a él.
#
# Verificamos no que «un objeto fue creado», sino que el clúster funciona en esencia:
#   1) el clúster lab responde por tu archivo de acceso (KUBECONFIG=~/lab.kubeconfig),
#   2) al menos un nodo está en estado Ready,
#   3) los nodos tienen recursos libres para futuras aplicaciones.
# Si COZY_TENANT está definido — adicionalmente miramos el clúster de GESTIÓN para comprobar que el
...
#     export COZY_TENANT=workshopXX      # para las verificaciones del lado del tenant (opcional)
...
# Sin acceso al propio clúster lab no hay nada que verificar — esta es la principal
# prueba del lab. need_kubeconfig detendrá el script con una pista clara,
...
# need_kubeconfig ya confirmó que el servidor responde. Registramos esto como un
...
# se levantó, pero el grupo de nodos md0 todavía se está desplegando.
...
if [ "${READY_NODES:-0}" -ge 1 ]; then
  ...
  fail "ningún nodo está en estado Ready (nodos en total: ${TOTAL_NODES:-0})" \
       "espera un par de minutos a que el grupo de nodos md0 se despliegue; el estado está en el dashboard de la aplicación lab, o: kubectl get nodes"
  evidence "Nodos del clúster" "${NODES_WIDE:-sin nodos}"
fi

# --- 3) ¿Hay espacio para futuras aplicaciones? ------------------------------
# allocatable del primer nodo: si no hay recursos, más adelante nada se ejecutará.
...
if [ -n "$ALLOC_MEM" ]; then
  ok "los nodos tienen recursos para aplicaciones (en el nodo: ${ALLOC_CPU} CPU, $(human_bytes "$ALLOC_MEM") RAM)"
...
# No es obligatorio para el lab 0: la conexión al propio clúster de arriba ya probó todo.
...
if [ -n "${COZY_TENANT:-}" ]; then
  ...
  if [ ! -r "$COZY_KUBECONFIG" ]; then
    warn "el acceso de tenant ${COZY_KUBECONFIG} no se encontró — el pedido del clúster en el lado de gestión no se verificó" \
         "esto no es un fallo del lab; define la ruta con: export COZY_KUBECONFIG=~/.kube/workshop"
  else
    ...
    if [ "$LAB_READY" = "True" ]; then
      ...
      warn "el pedido Kubernetes/lab aún no está Ready (actualmente: ${LAB_READY})" \
           "el clúster ya responde, la plataforma todavía lo está reconciliando al estado deseado; mira: kubectl --kubeconfig ~/.kube/workshop -n ${TENANT_NS} get kubernetes.apps.cozystack.io lab"
    else
      warn "no se encontró el pedido Kubernetes/lab en el tenant ${TENANT_NS}" \
           "si nombraste el clúster de otra forma — sustituye tu propio nombre; o tu rol en el tenant no permite este comando (no es un error del lab)"
    fi
    ...
    if [ "$MON" = "true" ]; then
      ...
      warn "la recolección de métricas está desactivada — el lab 14 quedará sin datos" \
           "para activar: dashboard → aplicación lab → Addons → Monitoring agents (las métricas no aparecerán retroactivamente)"
    fi
  fi
else
  warn "COZY_TENANT no está definido — las verificaciones del lado del clúster de gestión se omiten" \
       "no es obligatorio para el lab 0; para activar: export COZY_TENANT=workshopXX"
fi
