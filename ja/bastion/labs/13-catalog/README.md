...
| **必要なもの** | bastion 上の `helm`、`kubectl`、テナントアクセス。ここでは `lab` クラスターは不要 |
...
エラーは bastion 上で0.5秒で捕まえられます。スキーマがなければ、これはクラスターへ飛んで
...
📍 **場所:** bastion 上（そのターミナルで）。これにクラスターは不要です。
...
# client は、チェック全体が bastion 上で走り、クラスターはそれについて一切耳にしないという意味です。
