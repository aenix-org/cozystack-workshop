...
|---|---|---|
| `~/.kube/workshop` | Der Cozystack-Management-Cluster, Ihr Tenant | Redis ansehen: Adresse, Zustand |
...
if cache != nil {
    ...
    switch {
    case err != nil:
        log.Printf("Cache nicht erreichbar (%v), gehe ins Verzeichnis", err)
    case found:
...
lässt es auf Ihrem Laptop, `push` schickt es in die Registry. Der Image-Name ist derselbe wie im vorigen
...
⚠️ **`--platform linux/amd64` ist erforderlich, wenn Sie einen Mac mit Apple Silicon oder einen Laptop mit
...
📍 **Wo:** auf Ihrem Laptop, der `lab`-Cluster.
...
```json
{"cache":"off","cached":false,"dept":"Logistik","id":"42","name":"Pohl E.",
...
```bash
# port-forward svc/fortio = ein Tunnel von Ihrem Laptop zum Service des Generators im Cluster
#   8081:8080 — die linke Zahl ist der Port auf Ihrem Laptop, die rechte der Service-Port im Cluster
...
📍 **Wo:** auf Ihrem Laptop, der **Management**-Cluster.
...
Jetzt — eine Verbindungsprüfung. 📍 **Wo:** auf Ihrem Laptop, der **`lab`**-Cluster.
...
```
Cache nicht erreichbar (redis: NOAUTH Authentication required.), gehe ins Verzeichnis
Cache nicht erreichbar (redis: NOAUTH Authentication required.), gehe ins Verzeichnis
```
...
# Sie können sie nicht setzen: dann sucht das Skript ~/.kube/workshop, und findet es das nicht — überspringt
