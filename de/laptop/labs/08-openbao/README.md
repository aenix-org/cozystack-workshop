...
> Planen Sie es als eine durchgehende Sitzung ein, ohne Pause in der Mitte.
...
Repositorys liegt auf dem Laptop eines Auftragnehmers, der letztes Jahr eine Integration gemacht hat.
...
```
08:14:31 verbinde mich mit passes-db.internal als passes_app, Passwort-Fingerabdruck sha256:a609df223d57
```
...
Die kubeconfig des Tenants nehmen Sie aus dem Dashboard: **Info → Reiter Secrets →
`kubeconfig-tenant-workshopXX`**. Speichern Sie sie unter `~/.kube/workshop`.
...
Sie müssen es nicht auf dem Laptop installieren. Setzen Sie Ihre eigene Tenant-Nummer ein:
...
⚠️ **Kopieren Sie beide Werte jetzt sofort in eine Datei auf dem Laptop** — etwa in
...
mit dem Laptop abgeflossen — übermorgen ist er nutzlos. Ein Passwort in einem Manifest hat diese
Eigenschaft nicht.
...
```
Passwort aus OpenBao bezogen, im Manifest nicht vorhanden
```
