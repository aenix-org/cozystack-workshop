...
Mitarbeiter einen Gästeausweis anfordert. Vorerst bauen sie ihn auf dem Laptop eines einzelnen
...
Die Tenant-Zugangsdatei (das kubeconfig) holen Sie im Dashboard: **Info → der Reiter Secrets →
`kubeconfig-tenant-workshopXX`**, und speichern sie unter `~/.kube/workshop`. Das ist derselbe
Pfad wie in allen anderen Labs.
...
📍 **Wo:** auf dem Laptop, im selben Terminalfenster, in dem Sie mit `kubectl` gearbeitet
haben.
