...
#
# Wichtige Eigenschaft: Das Skript FÜHRT NICHTS ERNEUT AUS. Es nimmt das, was jedes Lab
# in dem Moment aufgezeichnet hat, als Sie es eingereicht haben. Andernfalls käme Folgendes
# heraus: Die Labs selbst weisen Sie an, hinter sich aufzuräumen, und die Tenant-Quota lässt
# es nicht zu, alle Dienste wochenlang laufen zu lassen — eine abschließende Nachprüfung würde
# also einen Fehlschlag für Arbeit anzeigen, die vor drei Wochen ehrlich erledigt wurde.
#
#   ./bundle.sh                 alles Gefundene sammeln
#   ./bundle.sh --rerun 07-redis  ein einzelnes Lab erneut durchlaufen und sein Ergebnis aktualisieren
#
set -uo pipefail
...
if [ "${1:-}" = "--rerun" ]; then
  ...
  echo "durchlaufe $lab erneut…"
  ( cd "$REPO/labs/$lab" && ./check.sh )
  echo "Ergebnis aktualisiert, führen Sie jetzt ./bundle.sh ohne Optionen aus"
  exit 0
fi
...
Schließen Sie mindestens eines ab und führen Sie die Prüfung aus — kommen Sie dann hierher zurück.

Wenn Sie die Labs auf einem anderen Rechner durchgeführt haben, kopieren Sie von dort den Ordner
~/.cozystack-labs/results oder geben Sie den Pfad an: COZY_LAB_RESULTS=/pfad ./bundle.sh
MSG
  exit 1
fi
...
# Das sechzehnte ("Was am Montag zu tun ist") hat kein Skript: Es ist eine Textübung,
# und es zählt nicht für die Bewertung.
all_labs = sorted(
...
for path in sorted(glob.glob(os.path.join(results_dir, "result-*.json"))):
    try:
        ...
        problems.append(f"{os.path.basename(path)}: kann nicht gelesen werden ({exc})")
        continue
...
# Dasselbe als Klartext — damit eine Person sehen kann, was sie sendet.
txt = out_path.rsplit(".", 1)[0] + ".txt"
with open(txt, "w") as fh:
    fh.write("Cozystack-Laborergebnisse\n")
    ...
    fh.write("\nNur Prüfungskennungen und ihre Ergebnisse gelangen in die Datei.\n"
...
if len(uids) > 1:
    print("\nAchtung: Die Ergebnisse stammen von %d verschiedenen Clustern. Das ist zulässig, "
          "aber der Satz wird beim Hochladen markiert." % len(uids))
for p in problems:
    print("  Problem: %s" % p)
print("\nDatei zum Hochladen: %s" % out_path)
print("Dasselbe als Klartext:  %s" % txt)
