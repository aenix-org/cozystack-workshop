...
Der Management-Zugang zum Tenant ist auf diesem Bastion bereits eingerichtet — die Datei
`~/.kube/config` (tokenbasiert, es öffnet sich kein Browser). Es gibt nichts zu holen oder zu
speichern.
...
und `$CH_AUTH` wird **innerhalb des Pods** expandiert, aus der Variable des Pods. Ihr Bastion sieht
...
innerhalb des Servers. Die Daten liefen nicht über das Netz, gingen nicht durch Ihren Bastion und
