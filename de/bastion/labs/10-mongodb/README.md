...
auch die Zugangsdatei die des Tenants sein muss. Auf dem Bastion ist sie bereits eingerichtet —
`~/.kube/config` (derselbe Pfad in jedem Lab, tokenbasierter Zugriff, es öffnet sich kein Browser).
...
#   --kubeconfig ~/.kube/config  welche Zugangsdatei verwendet wird. Ohne sie nimmt kubectl
...
sitzt `mongosh` — die MongoDB-Kommandoshell — und Sie müssen sie nicht auf dem Bastion installieren.
