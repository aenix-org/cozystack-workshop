...
# app-VM में root के रूप में netfix-dhcp.sh + रीबूट के बाद चलाएँ (अन्यथा DNS रिज़ॉल्व नहीं होगा)।
set -e
CONF=/etc/orders/application.properties

# =========================================================================
# अपने managed-सेवाओं के नाम यहाँ डालें।
#   आपके टेनेंट में Service DNS का फ़ॉर्मैट (namespace = tenant-<आपका-लॉगिन>):
#     Postgres: postgres-<postgres-नाम>-rw.<namespace>.svc.cozy.local
#     Kafka:    kafka-<kafka-नाम>-kafka-bootstrap.<namespace>.svc.cozy.local
#   सेवाओं के नाम डैशबोर्ड में दिखते हैं: Postgres/Kafka -> आपका इंस्टेंस -> Services।
# =========================================================================
# डिफ़ॉल्ट मान manifests/04-managed.yaml से मेल खाते हैं (Postgres=db, Kafka=kafka)।
# केवल tenant-workshopXX को अपने namespace से बदलें। यदि आपने Postgres/Kafka को अलग नाम दिया —
...
echo "== बाद में (managed DNS) =="
grep -E 'datasource.url|bootstrap-servers' "$CONF"

echo "== एप्लिकेशन को पुनः आरंभ कर रहे हैं =="
