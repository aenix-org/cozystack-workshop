# वर्कशॉप: VMware VM को Cozystack में माइग्रेट करना (bastion के ज़रिए)
...
**यह साझा VM (bastion) के ज़रिए वाला रास्ता है।** आपको अपने लैपटॉप पर कुछ भी इंस्टॉल करने की
ज़रूरत नहीं: `kubectl`, `virtctl` और `git` पहले से bastion पर मौजूद हैं, और वहाँ क्लस्टर तक
आपकी पहुँच पहले से सेट है। आप उसमें SSH करते हैं और वहीं काम करते हैं, फिर तैयार एप्लिकेशन को
ब्राउज़र में उसके डोमेन नाम से खोलते हैं।

> अगर आप अपने लैपटॉप से काम करते हैं (टूल्स खुद इंस्टॉल करके, एप्लिकेशन तक `port-forward` के
> ज़रिए पहुँचकर) — तो आपको दूसरा सेट चाहिए, [`../laptop/`](../laptop/)।
...
|---|---|---|
| 1 | इमेज के लिए स्टोरेज सेट करना | bastion पर |
| 2 | डिस्क को VMware फ़ॉर्मैट से KVM फ़ॉर्मैट में फिर से पैक करना | एक अस्थायी मशीन में |
| 3 | मशीन को उसके नए घर में चालू करना | bastion पर |
| 4 | कैटलॉग से डेटाबेस और क़तार ऑर्डर करना | bastion पर |
...
एक यूज़रनेम और एक पासवर्ड — तीनों जगह एक ही:

* **डैशबोर्ड** https://dashboard.workshop.aenix.io — ब्राउज़र से लॉग इन करें, namespace `tenant-workshopXX`
* **bastion** — SSH से लॉग इन करें: `ssh workshopXX@<bastion-address>`
* bastion के अंदर, क्लस्टर तक पहुँच पहले से सेट है, और kubeconfig `~/.kube/config` में है

नीचे हर जगह, `workshopXX` को अपने नंबर से बदलें (प्रशिक्षक ने आपको दिया है)।

## bastion में लॉग इन करना

```bash
ssh workshopXX@<bastion-address>
```

पासवर्ड वही है जो डैशबोर्ड के लिए है। किसी SSH key की ज़रूरत नहीं: लॉगिन पासवर्ड से होता है।
चलिए जाँचते हैं कि क्लस्टर तक पहुँच मौजूद है (यहाँ कोई ब्राउज़र नहीं खुलता — bastion सीधे टोकन
एक्सेस के लिए सेट है, Keycloak के बिना):
...
**आपको दिखना चाहिए:** कॉन्टेक्स्ट नाम `tenant-workshopXX` और मशीनों की (अभी खाली) सूची।

## सामग्री पहले से bastion पर है

क्लोन करने के लिए कुछ नहीं — सामग्री का फ़ोल्डर आपकी होम डायरेक्टरी में है, और मैनिफ़ेस्ट व
स्क्रिप्ट में आपका टेनेंट नंबर **पहले से भर दिया गया है**: bastion तैयार करते समय
`tenant-workshopXX` प्लेसहोल्डर को आपके `tenant-workshopNN` से बदल दिया गया था। खोजने और बदलने
के लिए कुछ नहीं — बस फ़ाइलों को जैसी हैं वैसे ही लागू करें।

```bash
cd ~/workshop
ls manifests scripts
grep -rl tenant-workshop manifests | head -1 | xargs grep -m1 namespace   # आपको अपना नंबर दिखेगा
```

एक जगह जानबूझकर प्लेसहोल्डर के रूप में छोड़ी गई है: `manifests/03-app-vm.yaml` में लाइन
`url: "ВСТАВЬТЕ_PRESIGNED_URL"` — वह लिंक आपको दूसरे चरण के बाद मिलेगा और आप उसे खुद भरेंगे।
...
📍 पहले bastion पर, फिर अस्थायी मशीन के अंदर।

VMware से आई डिस्क VMDK फ़ॉर्मैट में लिखी है, जबकि KVM QCOW2 पढ़ता है। `virt-v2v` फिर से पैक करने
का काम संभालता है; एक बार के काम के लिए इसे bastion पर इंस्टॉल करने का कोई मतलब नहीं, इसलिए हम
...
अंदर: `nano convert.sh`, `scripts/convert.sh` का टेक्स्ट पेस्ट करें, और `ВСТАВЬТЕ_...` की जगह
अपने `bucketName`, `accessKey` और `secretKey` डालें।

⚠️ **कन्वर्ज़न को `screen` के अंदर चलाएँ** — इसमें लगभग पाँच मिनट लगते हैं, और अगर bastion तक
आपका SSH सत्र टूट जाए, तो सामान्य रन बीच में ही कट जाएगा। `screen` प्रोसेस को ज़िंदा रखता है, तब
भी जब कनेक्शन चला गया हो:

```bash
screen -S convert          # एक अलग सत्र खोलें
sudo bash convert.sh       # उसी सत्र के अंदर चलाएँ
#  कनेक्शन टूट गया? bastion में वापस ssh करें, फिर:  screen -r convert
```
...
**आपको दिखना चाहिए:** `200`। अगर `503` — डेटाबेस या क़तार से कुछ कनेक्ट नहीं हुआ। यहाँ
`localhost` वही मशीन है जिसमें आप बैठे हैं: एप्लिकेशन को अंदर से जाँचा जा रहा है।
...
हम स्कीमा लाते हैं और उसे लागू करते हैं (यह app-VM इंटरनेट तक पहुँचती है, इसलिए फ़ाइल डाउनलोड होगी):
...
### स्टेप 3. बाहर से जाँच — डोमेन नाम से

📍 अपने लैपटॉप पर ब्राउज़र में, या bastion पर `curl` के ज़रिए।

यहीं इस रास्ते का मुख्य अंतर सामने आता है: **किसी port forwarding की ज़रूरत नहीं।** प्रशिक्षक ने
आपके tenant में पहले ही एक `Ingress` बना दिया है, और जैसे ही मशीन के अंदर एप्लिकेशन `8080` पर सुन
रहा होता है, दुकान `https://app.workshopXX.workshop.aenix.io` पर प्रकाशित हो जाती है (`XX` आपका
नंबर है)। इसे वहीं से जाँचें:
...
⚠️ जब तक app-VM चालू नहीं हुई या अभी भी बूट हो रही है, डोमेन `503` जवाब देता है — यह सामान्य है:
`Ingress` किसी backend का इंतज़ार कर रहा है। मशीन के शुरू होते ही (अंदर `8080` पर सुनते हुए) यह
`200` हो जाता है।
...
> **हर कमांड को `vmi/` प्रीफ़िक्स की ज़रूरत नहीं, और यह कोई टाइपो नहीं है।** tenant अनुमतियों के
> तहत, `virtctl console` सिर्फ़ **सादा** नाम स्वीकार करता है (`vm-instance-app-1`); `vmi/` के
> साथ यह `forbidden` जवाब देता है, `vmi` शब्द को मशीन का नाम समझते हुए। इसके विपरीत,
> `virtctl ssh` और `virtctl port-forward` को `vmi/<name>` रूप चाहिए।
...
# SSH के ज़रिए app-VM के अंदर एक शेल (मशीन का नेटवर्क चालू होने के बाद)
virtctl ssh ubuntu@vmi/vm-instance-app-1 --namespace=tenant-workshopXX
```

आप एप्लिकेशन को डोमेन से जाँचते हैं, `https://app.workshopXX.workshop.aenix.io`; इस रास्ते पर
`port-forward` की ज़रूरत नहीं। कंसोल से निकलने के लिए — `Ctrl+]`। अगर कनेक्ट करने के बाद स्क्रीन
खाली है, तो Enter दबाएँ। वही चीज़ माउस से भी उपलब्ध है: डैशबोर्ड में मशीन के पेज पर **VNC** बटन।
...
* लंबे कन्वर्ज़न को `screen` के अंदर चलाएँ — वरना SSH टूटने पर यह बीच में कट जाएगा।
...
27 संदेशों की सूची — [chat/README.md](chat/README.md)।
