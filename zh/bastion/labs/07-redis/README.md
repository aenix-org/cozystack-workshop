...
|---|---|---|
| `~/.kube/config` | Cozystack 管理集群，你的租户 | 查看 Redis：地址、状态 |
...
📍 **位置：** 在 bastion（跳板机）上（在 bastion 终端里）。
...
if cache != nil {
    ...
    switch {
    case err != nil:
        log.Printf("缓存不可用（%v），改为访问目录服务", err)
    case found:
...
构建它并推送到你自己的 Harbor。`build` 会根据 `Dockerfile` 构建镜像并把它留在 bastion 上，`push` 会把它发送到镜像仓库。镜像名和上一个实验一样，但标签不同——`v2`：现在镜像仓库里会同时存放两个版本，旧的哪儿也不会去。
...
```bash
# port-forward svc/fortio = 从 bastion 到集群里生成器服务的一条隧道
#   8081:8080 — 左边的数字是你 bastion 上的端口，右边是集群里服务的端口
...
现在——做连通性检查。📍 **位置：** 在 bastion 上，实验集群 **`lab`**。
...
```
缓存不可用（redis: NOAUTH Authentication required.），改为访问目录服务
缓存不可用（redis: NOAUTH Authentication required.），改为访问目录服务
```
...
📍 **位置：** 在 bastion 上，就在你用 `kubectl` 工作过的那个终端窗口里。
...
# 可以不设:那样脚本会去找 ~/.kube/config，找不到就跳过
# 对管理集群的检查，并会告诉你这一点。
