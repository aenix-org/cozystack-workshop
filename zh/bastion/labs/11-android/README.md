...
移动团队正在给 Propusk 写一个客户端——就是员工用来给访客申请通行证的那个界面。目前他们把它构建在某个开发者的 VM 上。等这个人休假了，就没人能发版。
...
这台 bastion 上访问租户的文件已经配好了——`~/.kube/config`，跟所有实验里都是同一个路径（基于令牌的访问，不会打开浏览器）。
...
这个应用只做一件事：它显示一行字«Заказать пропуск для гостя»。这就够了，因为这个实验讲的不是 Android，而是它在哪里被构建出来。
...
| 日志标记 | 大约在什么时候 |
|---|---|
| `== 1/5 ставлю Android command-line tools ==` | 立刻 |
| `== 2/5 принимаю лицензии и качаю SDK (самый долгий шаг) ==` | +1–2 分钟，而且卡得最久 |
| `== 3/5 собираю APK ==` | 从开始起 +5–15 分钟 |
...
📍 **位置：** 在 bastion 上，在你之前用 `kubectl` 干活的那个终端窗口里。
