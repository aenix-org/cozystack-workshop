...
从 bastion 通过 `port-forward` 去压这个应用是没有意义的：瓶颈会变成你家里的网络和隧道本身，而不是应用。生成器必须待在集群内部，紧挨着目标。
...
# port-forward 从 bastion 拉一条隧道进集群。
...
```bash
# exec 在一个已经运行的 Pod 内部执行一条命令，而不是在你的 bastion 上。
...
📍 **位置：** 在 bastion 上，在你此前用 `kubectl` 干活的同一个终端窗口里。
