...
```bash
# port-forward = 从 bastion 挖一条隧道通进集群。命令运行期间，
# 对 localhost:8081 的请求会落到负载生成器上。
#   svc/fortio    目标：名为 fortio 的 service（不是 pod）
#   8081:8080     左边是你 bastion 上的端口，右边是 service 内部的端口
...
📍 **在哪里做：**在 bastion 上，就是你刚才操作 `kubectl` 的那个终端窗口里。
...
export COZY_KUBECONFIG=~/.kube/config     # 访问租户的文件
