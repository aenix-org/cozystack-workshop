...
| **你需要什么** | 访问租户控制台的权限；bastion 上的 `kubectl`（已安装好） |

> **从你自己的 bastion 或笔记本电脑。** 讲师提供的虚拟机上一切都已装好——本段可以跳过。如果你在自己的机器上做实验，请提前装好：`kubectl`、`helm`（v3+）、`virtctl`（用于虚拟机实验，例如实验 12）、`git`、`python3`、`curl`，并获取你租户的 `kubeconfig`（控制台 → 下载访问凭据）。没有 `helm`，实验 13 无法通过；没有 `virtctl`，实验 12 里的控制台打不开。
...
⚠️ **如果 WSL 被安全策略禁用了**——在企业的 bastion 上这很常见——实验依然能做：凡是在控制台里完成的一切都与操作系统无关。你唯一没法做的，只有运行检查脚本和少数几个完全由命令组成的步骤。这些地方会单独标出。
...
## 步骤 0. 材料已经在 bastion 上

📍 **在哪里：** 在 bastion 上（在 bastion 终端里）。

清单（manifest）——描述要在集群里创建什么的文件——和检查脚本，已经躺在你的主目录里，在 `~/workshop` 目录下，而且你的租户编号已经替换进去了。没什么要克隆的——我们进去看看里面有什么：
...
从这里往后，实验里每个路径都从这个文件夹（`~/workshop`）算起。

## 拿到平台的访问权限

📍 **在哪里：** 在浏览器里，然后在 bastion 上。

你从平台订购的一切，都存在于**管理集群**上——和你的租户在同一个地方。要用命令连接它，你需要一个访问文件。在这台 bastion 上它**已经配置好了**——`~/.kube/config`。访问是基于令牌的，所以你与集群打交道时浏览器不会打开，Keycloak 也不会问你任何东西。

这个路径在每个实验里都会用到。我们来检查访问是否可用：

```bash
# 向管理集群索要你的 Kubernetes 集群列表。
# --kubeconfig 显式指向访问文件（这里它也正好是默认文件）。
...
📍 **在哪里：** 在 bastion 上；文件本身用命令取，或从控制台取。

**我们要取什么。** `lab` 集群的 kubeconfig——一个文本文件，里面记着它 API server 的地址和你对它的访问数据。没有这样一个文件，`kubectl` 就不知道去哪里连接、以谁的身份出示自己。这个文件你自己在 bastion 上创建，名字叫 `~/lab.kubeconfig`；路径里的 `~` 就是你的主目录：macOS 上是 `/Users/name`，Linux 和 WSL 上是 `/home/name`。

⚠️ **这是第二个访问文件，不是第一个的替代品。** 连同租户一起发给你的那个（在实验里它躺在 `~/.kube/config` 路径下）通往管理集群——你在那里订购应用、也刚在那里创建了 `lab`。新的这个文件通往 `lab` 集群内部本身。这是两个地址不同的不同集群，而从这里往后两个都需要：给平台的订单走第一个文件，在你自己集群内部的工作走第二个。

**它躺在哪里。** 平台把它放进了你租户里的 Secret `kubernetes-lab-admin-kubeconfig`。Secret 是一种集群对象，里面存放密码、密钥和访问文件。Secret 里你需要的那个键是 `admin.conf`。

⚠️ **这个 Secret 里有四个键，而你要的正是 `admin.conf`。** 挨着它的是 `admin.svc`——同一个东西，但带的是只在集群内部可见的内部地址；从 bastion 用它连不上。`super-admin.*` 这一对授予绕过既定限制的权限，是为事故之后的处理准备的，而不是日常使用。

**主要办法——用命令。** Cozystack 在你的集群上设置了一条单独的访问规则，只允许读这一个 Secret，别的什么都不许。命令在**管理集群上**运行，用连同租户一起发给你的那份访问权限，结果放进 bastion 上的一个文件里：
...
📍 **在哪里：** 在 bastion 上，就在你刚才用 `kubectl` 工作的那个终端窗口里。
