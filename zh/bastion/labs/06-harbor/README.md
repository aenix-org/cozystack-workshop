...
| **需要准备** | 实验 0 建好的集群、bastion 上的 `kubectl`、`docker`（或 `podman`）、控制台访问权限 |
...
|---|---|---|
| `~/.kube/config` | Cozystack 管理集群，你的租户 | 查看托管服务：Harbor、数据库、队列 |
...
📍 **在哪里：** 在控制台（dashboard）里，然后在 bastion 上的终端里。
...
现在从终端做同样的事。`docker login` 会询问用户名和密码，并把凭据保存在你的 bastion 上，
...
⚠️ **这次 `docker login` 教会了你的 bastion 登录镜像仓库 —— 而且仅仅是它。** 它对集群没做
...
`docker build` 命令构建镜像：它读取 `Dockerfile`，执行其中描述的步骤，并把结果放进你 bastion
上的镜像存储里。结果存放时用的名字由 `-t` 标志指定，由三部分组成：
...
#                           而 bastion 可能是 ARM —— 那样不加这个标志就会构建出不对的东西
...
ARM 的 bastion，不加这个标志你就会构建出一个 ARM 镜像。它构建时不报错，推送时不报错，
...
你的 `docker login` 把凭据写进了**你 bastion 上**的文件 `~/.docker/config.json`。集群节点对
这个文件一无所知，也不可能知道：它和你的 bastion 之间根本没有任何共同点，除了你会往那里
发命令这一点之外。
...
# base64 -d 把值解码回文本 —— 你会看到和 bastion 上
...
应用没有对外暴露，但你需要看看它。`port-forward` 从 bastion 往集群里挖一条隧道：命令运行
...
#   8080:80 —— 左边的数字是 bastion 上的端口，右边是集群里 service 的端口
# 别关窗口：命令运行多久，隧道就存活多久。
...
📍 **在哪里：** 在 bastion 上，在你刚才使用 `kubectl` 的同一个终端窗口里。
...
# 也可以不设：那样脚本会去找 ~/.kube/config，找不到就跳过
# 管理集群上的检查，并作出说明。
...
- 把 bastion 上的 `docker login` 和集群需要的访问权限区分开
