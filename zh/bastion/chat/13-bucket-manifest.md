...
`namespace: tenant-workshopXX` —— 你在平台上的那一块地盘。在 bastion 上的这个文件里，**你的编号已经填在这里了**——它是在准备测试环境（testbed）时填入的，无需做任何改动（`XX` 只是作为示例展示）。namespace（命名空间）是集群内部的一道隔断：不同 namespace 里同名的对象互不干扰、彼此看不见。最接近的类比是一个独立的 Resource Pool，带着自己的访问权限，只是更严格。
