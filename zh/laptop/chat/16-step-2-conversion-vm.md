...
**为什么要单独一台机器，而不是你的笔记本。** 这个工具叫 `virt-v2v`，它会拖来一大堆依赖，跑在 Linux 上，还要吃掉几十个 GB。为了用一次就把它装在你的工作笔记本上是个糟糕的主意，而在 Windows 和 macOS 上它根本跑不起来。更省事的做法是在存储旁边启动一台用完即弃的机器，在里面把活干完，然后关掉。
