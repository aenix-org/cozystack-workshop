...
| **需要什么** | 笔记本电脑上的 `helm`、`kubectl`、租户访问权限。这里不需要实验集群 `lab` |
...
错误在笔记本电脑上半秒内就被抓住了。没有 schema 的话，它会一路开到集群，变成一个永远建不出来的 Deployment，还带着三屏长的消息。
...
# client 意味着整个检查都在笔记本电脑上进行，集群甚至不会听说这件事。
...
📍 **在哪：** 在笔记本电脑上，在你之前用 `kubectl` 干活的同一个终端窗口里。
