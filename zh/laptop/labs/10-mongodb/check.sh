#!/usr/bin/env bash
# 实验 10 检查：MongoDB 中存放着不同形态的通行证，并按其进行检索。
#
# 我们检查的不是「服务已创建」，而是本质：集合中存在全部四种形态的文档，
# 按嵌套字段以及深入列表内部的检索都能工作，在稀有字段上建立了稀疏索引，
# 模式校验器已启用，而且没有遗留缺少类型的文档。
#
# 运行（每开一个新的终端窗口都要重新设置变量）：
...
#
# 密码不会被打印，也不会出现在报告里。
# 脚本会启动一次性的 Pod，因此大约需要一分钟。

# 名称和标题是公共库所需要的：它用它们来给报告工件签名。
# lib.sh 里存放着 ok/fail/warn/evidence/finish 以及下面的环境检查——这样
# 十五个检查脚本就能以统一的方式打印，而不是各行其是。
LAB_NAME="10-mongodb"
LAB_TITLE="实验 10 · 文档存储"
. "$(cd "$(dirname "$0")/../../check" && pwd)/lib.sh"

# 如果没有设置集群访问文件或租户编号，这两个检查都会以清晰的消息停止脚本。
# 否则后面 kubectl 的错误会不断堆积。
need_kubeconfig
need_tenant

# 参与者把 COZY_TENANT 设成 `workshop07`，而 namespace 叫做
...
# 默认名称与实验里的相同。写法 ${X:-值} 的意思是「取环境变量，
# 如果它不存在，就代入这个值」：如果你给应用起了别的名字——
# 就用 MONGO_APP=名称 ./check.sh 来运行，无需修改脚本。
# 地址是内部的，来自集群自身；名字里的 rs0 是副本集，我们唯一的那份副本就住在里面。
...
# --- 1. 到端口究竟通不通 -----------------------------------------------------
# MongoDB 在自己的端口上会用一句清晰的话回应 HTTP 请求，说明这里应该
# 用驱动来访问，而不是浏览器。这足以把
# 「名字解析不了 / 端口关闭」和「连接是通的，只是凭据不对」区分开。
...
if printf '%s' "$PROBE" | grep -qi 'mongodb'; then
  ...
  fail "在地址 ${MONGO_HOST} 上连不上 MongoDB" \
       "检查 COZY_TENANT 里的租户编号和应用名称（默认 'passes'；否则用 MONGO_APP=名称 ./check.sh）；在仪表盘中应用应处于就绪状态"
  finish
  exit $?
fi

# 后面的一切都需要登录数据库。没有密码时脚本既不瞎猜也不沉默，
# 而是老实说明数据库内容未经检查，并结束报告：否则
# 参与者会以为检查已经通过。
if [ -z "${MONGO_PASSWORD:-}" ]; then
  fail "未设置 MONGO_PASSWORD 变量，数据库内容未经检查" \
       "export MONGO_PASSWORD='${MONGO_USER} 用户的密码'，然后重新运行脚本"
  finish
  exit $?
fi

# 密码要做百分号编码：其中的 @ : / ? # % 字符否则会把连接字符串
# 拆坏，人得到的将是一个含糊的解析错误，而不是「密码错误」。
...
# ⚠️ 连接字符串包含密码，并作为 Pod 参数传入。这是一个有意的
# 折中：见 check/lib.sh 里的 `in_cluster_with_secrets`——安全的路径是有的，但
# 它在不过度复杂化的前提下无法兼容多行的 --eval。Pod 只存活几秒并会自我清理；
# 密码不会进入报告。在生产脚本里不要这么做。
#
# 所有检查一次跑完：每次调用都会启动一个 Pod，连续十个 Pod
# 会平白无故地把检查变成好几分钟的等待。
# 向外只输出一行 JSON，之后由 python 解析它。
# `--overrides` 带上 securityContext：没有它，在采用 `restricted` 配置的集群里
# Pod 就创建不了，实验会因为一个与参与者无关的原因而失败。
# `--command --` 保留：kubectl 会把它与 override 合并，而 override 里只设置了
# 安全相关的字段。
# 给 mongosh 用的程序。其中的双引号是安全的：文本向外经由
# python，它会自己把文本加上引号，而数据库名和集合名由下面的
# 标记代入。
...
# kubectl 把 override 作为 JSON merge patch 应用，其中 containers 数组会被整体替换：
# 在外面设置的 `--command` 到不了 Pod，于是启动的将不是 mongosh，而是
# 镜像的默认进程——也就是数据库本身。check/lib.sh 里也是这么做的。
...
# 从 mongosh 打印的 JSON 字符串里取出一个字段。列表用逗号
# 拼接，好让它们能原样展示给参与者。
mget() {
...
# 同样的，但针对数字：任何意外的值都会变成 0，否则下面的比较
# 就会以算术错误告终，而不是给出清晰的 FAIL。
num() {
  local v
  v="$(mget "$1")"
  case "$v" in
...
# 如果根本没有回应，或者 mongosh 报告了错误——就没什么可继续检查的了。
# 认证被拒绝与其他错误分开处理：它有自己常见的原因——
# 忘了 authSource=admin，而提示应该正好指向它。
if [ -z "$SUMMARY" ] || [ "$(mget ok)" != "1" ]; then
  ERR="$(mget error)"
  case "$ERR" in
    *[Aa]uthentication*)
      fail "MongoDB 未接受用户 ${MONGO_USER} 的凭据" \
           "检查密码，以及连接字符串中是否有 authSource=admin：用户建在 admin 数据库里，而权限授予在 ${MONGO_DB} 里" ;;
    *)
      fail "无法对数据库 ${MONGO_DB} 执行查询${ERR:+: $ERR}" \
...
ok "以用户 ${MONGO_USER} 连接数据库 ${MONGO_DB} 正常工作"

# --- 2. 文档存在 ------------------------------------------------------------
TOTAL="$(num total)"
if [ "$TOTAL" -ge 4 ]; then
  ok "集合 ${MONGO_COLL} 中的文档数：${TOTAL}"
else
  fail "集合 ${MONGO_COLL} 中一共只有 ${TOTAL} 个文档，预期不少于四个" \
       "加载通行证：mo < passes.js（文件说明见 README）"
fi
...
if [ "$TYPES" -ge 4 ]; then
  ...
  fail "不同类型的通行证一共只有 ${TYPES} 种，预期四种" \
...
if [ "$WITH_ARRAY" -ge 2 ]; then
  ...
  fail "带列表的文档有 ${WITH_ARRAY} 个，预期不少于两个" \
       "周卡和团体通行证没加载上；重新执行 mo < passes.js"
fi

NESTED="$(num nested)"
if [ "$NESTED" -ge 1 ]; then
  ok "深入对象列表内部的检索（members.name）能找到文档"
else
  fail "按 members.name 的检索什么都没找到" \
       "带成员列表的团体通行证没加载上；重新执行 mo < passes.js"
fi
...
稀疏：${SPARSE}"
  ...
  fail "没有稀疏索引——按车牌号的检索走的是全表扫描" \
       "创建它：db.${MONGO_COLL}.createIndex({ 'car.plate': 1 }, { name: 'car_plate', sparse: true })"
  evidence "集合索引" "全部：${IDX}"
fi
...
if [ "$VALIDATOR" = "1" ]; then
  ...
  fail "模式校验器未启用——字段名里的拼写错误会悄无声息地通过" \
       "启用它：mo < validator.js（可预见失败的详解见 README）"
fi
...
if [ "$TYPELESS" -eq 0 ]; then
  ...
  fail "集合中有 ${TYPELESS} 个缺少 type 字段的文档——保安看不到它们" \
       "找出并清除它们：db.${MONGO_COLL}.deleteMany({ type: { \$exists: false } })"
fi

# finish 打印结果并把报告工件存入文件；只要有一项检查失败，
# 返回码就是非零。
finish
