...
租户的 kubeconfig 从控制台获取：**Info → Secrets 标签页 → `kubeconfig-tenant-workshopXX`**。把它保存到 `~/.kube/workshop`。
...
`sh -c '…'` 用单引号括起来——字符串原样传进去，`$CH_AUTH` 是**在 Pod 内部**、从 Pod 的变量展开的。你的笔记本电脑看不到这些值，也不会把它们写进命令历史。
...
**这花了多长时间。** 一百万行在几秒内生成并写入，完全在服务器内部完成。数据没有走网络，没有经过你的笔记本电脑，也没有落在中间文件里。把它和造测试数据的惯常做法——一次插入一行的脚本——比一比。
...
📍 **在哪里：** 在笔记本电脑上，在你刚才用 `kubectl` 工作的同一个终端窗口里。
...
export CH_PASSWORD='你的-analyst-密码'  # 订购 ClickHouse 时你设的那个
./check.sh
```
