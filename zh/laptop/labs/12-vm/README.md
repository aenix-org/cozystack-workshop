...
| **需要准备** | 租户控制台（dashboard）访问权限、租户的 `~/.kube/workshop`、`kubectl`、`virtctl` |
...
📍 **在哪里：** 在你自己的笔记本上，另开一个终端窗口。或者干脆在浏览器里。
...
📍 **在哪里：** 在控制台里，在 `spravochnik` 机器的卡片上。
...
如果连接后屏幕是空的——按下 Enter，登录提示就会出现。退出控制台——`Ctrl+]`。你租户里所有正在运行的实例的名字，可用 `kubectl --kubeconfig ~/.kube/workshop get vminstance -n tenant-workshopXX` 查看。
...
📍 **在哪里：** 在你自己的笔记本上，就在你用 `kubectl` 干活的那个终端窗口。
...
如果你打算做监控那个实验，就把虚拟机留着：它的消耗在图表里也看得到，是个不错的示例。如果不打算——就通过控制台把机器和磁盘删掉。
