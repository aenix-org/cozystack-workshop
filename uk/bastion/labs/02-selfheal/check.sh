...
#
# Скрипт нічого не видаляє і не створює, крім одноразового Pod для перевірки
...
# Сенс лаби в тому, що копію повертає ReplicaSet, а не «кластер загалом».
...
# Прямих доказів «Pod убивали» кластер не зберігає. Є два непрямі, і обидва достатні:
...
if [ -n "$DEP_E" ] && [ -n "$POD_E" ]; then
  ...
  evidence "Вік об'єктів" "deployment створено: ${DEP_TS}
pod створено:        ${POD_TS}
різниця:           ${DELTA} с"
else
  warn "не вдалося порівняти вік Pod і застосунку" \
       "потрібен python3 у PATH; на проходження лаби це не впливає"
fi

# Події живуть близько години, тому їх відсутність — не провал, а зауваження.
...
if [ "$CREATES" -ge 2 ]; then
  ok "у подіях кластера ${CREATES} створення копії — самовідновлення дійсно спрацьовувало"
  ...
  warn "у подіях кластера видно створення копії лише ${CREATES} раз(ів)" \
       "події зберігаються близько години і могли сплинути"
fi
...
if [ -z "$BODY" ]; then
  ...
  if [ -z "$SERVED" ]; then
    ...
    fail "сторінку віддала копія ${SERVED}, але такого Pod в кластері вже немає" \
         "почекайте десяток секунд і запустіть перевірку знову — ймовірно, копія змінювалася прямо зараз"
  fi
fi
