...
#   - коридор задано осмислено (maxReplicas більше одного, інакше нема куди рости);
...
#
# Запускати ДО прибирання. Частина перевірок спирається на сліди вже наявного росту,
...
# Імена винесено у змінні, щоб збіг імені застосунку та імені HPA
...
# зрозумілої, тому тут єдине місце, де скрипт завершується достроково.
if ! kubectl get deployment "$APP" >/dev/null 2>&1; then
  fail "застосунку ${APP} немає в кластері — масштабувати нічого" \
...
# Найчастіша причина «HPA не працює», і по маніфесту її не видно:
...
if [ -n "$REQ_CPU" ]; then
  ok "у контейнера задано requests.cpu = ${REQ_CPU} — є від чого рахувати відсотки"
  evidence "Ресурси контейнера" "requests.cpu: ${REQ_CPU}
limits.cpu:   ${LIM_CPU:-не задано}"
else
  fail "у контейнера ${APP} не задано requests.cpu" \
       "HPA за Utilization без нього не працює; застосуйте ../01-deploy/rickroll.yaml заново"
fi

# --- сам HPA ---------------------------------------------------------------
# Перевіряємо не лише наявність об'єкта, а й на кого він націлений. HPA з опискою
# у scaleTargetRef створюється успішно і виглядає в списку як робочий, але всю лабу
...
if [ -n "$MAXR" ] && [ "$MAXR" -gt 1 ] 2>/dev/null; then
  ok "коридор задано: від ${MINR} до ${MAXR} копій — рости є куди"
else
  fail "верхня межа коридору дорівнює ${MAXR:-не задана} — рости нема куди" \
...
if [ "$TGT_TYPE" = "Utilization" ] && [ -n "$TGT_VAL" ]; then
  ok "поріг задано: ${TGT_VAL}% від requests.cpu (${REQ_CPU:-?})"
else
  warn "поріг задано не у відсотках від requests (тип: ${TGT_TYPE:-немає})" \
       "лаба розбирає варіант Utilization; на працездатність це не впливає"
fi

# --- ГОЛОВНЕ: метрики реально збираються -----------------------------------
# Саме тут видно різницю між «об'єкт створено» і «механізм працює».
...
if [ -n "$CUR_UTIL" ] && [ "$SCALING_ACTIVE" = "True" ]; then
  ok "метрики збираються: поточне завантаження ${CUR_UTIL}% від requests, HPA ухвалює рішення"
  ...
  fail "HPA не отримує метрики — у TARGETS буде <unknown>, вирішувати йому нема на чому" \
       "перші дві хвилини після apply це нормально, зачекайте і повторіть; якщо не минуло — kubectl top pods і kubectl describe hpa ${HPA}"
  evidence "Чому HPA не активний" "${REASON:-причину не вказано в статусі}"
fi

evidence "Стан HPA" "$(kubectl get hpa "$HPA" 2>/dev/null)"

# --- metrics-server відповідає напряму -------------------------------------
# Дублює попередню перевірку з іншого боку і розділяє дві різні поломки:
# «метрик немає в усьому кластері» і «метрики є, але HPA до них не дістався».
...
# від того, чи спливли події кластера, чи ні.
...
# Однієї позначки часу мало: вона проставляється і при скороченні копій, тобто
# з'являється навіть у того, хто підняв репліки руками і дав HPA прибрати зайві. Шукаємо
# саме ріст ЗА НАВАНТАЖЕННЯМ — подію з перевищенням порога.
#
...
if [ "${SCALE_UP:-0}" -ge 1 ]; then
  ...
  evidence "Масштабування" "подій росту: ${SCALE_UP}
...
# учасник дізнався про зникнення генератора тут, а не посеред викатки під навантаженням,
# коли зупинятися і розгортати його буде недоречно.
if kubectl get deployment fortio >/dev/null 2>&1; then
  ...
  if printf '%s' "$FBODY" | grep -qi 'fortio'; then
    ok "генератор навантаження Fortio працює і відповідає зсередини кластера"
  else
