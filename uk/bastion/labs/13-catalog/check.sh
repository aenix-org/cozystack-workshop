...
# чарт збирається, схема працює, визначення розібрано й узгоджено з чартом.
#
# Запуск із папки лаби:
#   cd labs/13-catalog && ./check.sh
# Кластер не обов'язковий: без KUBECONFIG дві перевірки буде пропущено з попередженням,
# а не з помилкою.
...
# --- лінтер ----------------------------------------------------------------
# helm lint читає чарт як текст: знаходить одруківки в шаблонах, відсутні поля
...
# --- рендер ----------------------------------------------------------------
# Порожній вивід і вивід із самих коментарів лінтер би пропустив, тому дивимося,
...
if printf '%s' "$RENDER" | grep -q '^kind: Deployment'; then
  ...
  fail "helm template не видав жодного Deployment" \
       "дивіться помилку рендеру: helm template main chart"
...
#                  саме те, об що чарт спотикається в замовника.
...
if [ -n "${KUBECONFIG:-}" ] && kubectl version -o json >/dev/null 2>&1; then
  ...
  # Відмова в правах і відмова за схемою — різні речі, і плутати їх не можна. Під тенантним
  # доступом (~/.kube/config) прав на Deployment і ConfigMap немає зовсім, тому сюди
  # прилетить Forbidden — і це нічого не каже про якість чарта. Перевірка по суті
  ...
  if printf '%s' "$DRY" | grep -qiE 'forbidden|cannot create|is not allowed'; then
    warn "серверну перевірку чарта пропущено: поточний доступ не дозволяє її виконати" \
  ...
  warn "перевірку чарта на кластері пропущено: немає доступу" \
...
if [ "${R5:-0}" -ge 1 ]; then
  ok "параметр replicas доходить до маніфеста (--set replicas=5 дає replicas: 5)"
else
  fail "параметр replicas не доходить до маніфеста" \
       "у templates/deployment.yaml має стояти replicas: {{ .Values.replicas }}"
fi
...
if helm template main "$CHART" --set replicas=abc >/dev/null 2>&1; then
  fail "схема значень не відхиляє свідомо неправильне значення (replicas=abc пройшло)" \
       "перевірте, що поряд із values.yaml лежить values.schema.json і в ньому replicas оголошено як integer"
else
...
# уже в себе, а розбиратися доведеться автору файлу.
if [ ! -f "$APPDEF" ]; then
  fail "не знайдено ${APPDEF}" \
       "файл має лежати поряд із чартом; візьміть його з репозиторію лаб"
else
  MISSING=""
  # Шукаємо ключі порядково, без розбору YAML: PyYAML є не на кожній машині,
  ...
  if [ -z "$MISSING" ]; then
    ...
    fail "в ApplicationDefinition не вистачає полів:${MISSING}" \
         "звірте з розбором у README — без будь-якого з них адмін отримає відмову при застосуванні"
  fi
...
if a == b:
    ...
    print("DIFF лише у визначенні: %s | лише в чарті: %s"
...
PY
  ...
  # розкодовуємо й дивимося, що всередині справді SVG.
...
Об'єкт cluster-scoped і змінює каталог для всіх тенантів, тому його застосовує адмін платформи."
  ...
  warn "кластер не опитано (KUBECONFIG не задано або не відповідає)" \
       "перевірка локальна, кластер тут не потрібен. Щоб побачити відмову в правах: export KUBECONFIG=~/.kube/config"
fi
