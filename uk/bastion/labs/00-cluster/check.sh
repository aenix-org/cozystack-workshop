...
# Якщо задано COZY_TENANT — додатково дивимося на КЕРУВАЛЬНОМУ кластері, що замовлення
...
# доказ лаби. need_kubeconfig зупинить скрипт зі зрозумілою підказкою,
...
# need_kubeconfig уже переконався, що сервер відповідає. Фіксуємо це окремим
# результатом і кладемо версію сервера у звіт.
KVER="$(server_version)"
...
# --- 4) З боку керувального кластера (якщо задано тенант) --------------------
...
if [ -n "${COZY_TENANT:-}" ]; then
  ...
  if [ ! -r "$COZY_KUBECONFIG" ]; then
    warn "тенантний доступ ${COZY_KUBECONFIG} не знайдено — замовлення кластера на керувальному не перевірялося" \
         "це не провал лаби; шлях задається: export COZY_KUBECONFIG=~/.kube/config"
  else
    LAB_READY="$(cozy get kubernetes.apps.cozystack.io lab -n "$TENANT_NS" \
      -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}')"
    if [ "$LAB_READY" = "True" ]; then
      ok "на керувальному кластері замовлення Kubernetes/lab у стані Ready"
    elif [ -n "$LAB_READY" ]; then
      warn "замовлення Kubernetes/lab ще не Ready (зараз: ${LAB_READY})" \
           "кластер уже відповідає, платформа ще зводить його до заданого; подивіться: kubectl --kubeconfig ~/.kube/config -n ${TENANT_NS} get kubernetes.apps.cozystack.io lab"
    else
  ...
  warn "COZY_TENANT не задано — перевірки з боку керувального кластера пропущено" \
