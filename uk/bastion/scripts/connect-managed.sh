#!/bin/bash
# Перемкнути мігрований застосунок з legacy hardcoded IP на managed DNS-endpoints.
# Запускати в app-VM під root ПІСЛЯ netfix-dhcp.sh + перезавантаження (інакше DNS не резолвить).
set -e
...
#     Postgres: postgres-<імя-postgres>-rw.<namespace>.svc.cozy.local
#     Kafka:    kafka-<імя-kafka>-kafka-bootstrap.<namespace>.svc.cozy.local
...
# Замініть лише tenant-workshopXX на свій namespace. Якщо називали Postgres/Kafka інакше —
# поміняйте db/kafka на свої імена.
