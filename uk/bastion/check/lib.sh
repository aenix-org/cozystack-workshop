...
#
# Навмисно НЕ використовується `set -e`: скрипт зобов'язаний прогнати всі перевірки і показати
# повну картину, а не зупинятися на першій же невдачі. Читач запускає його
...
# result-<лаба>.json збирається паралельно з людським звітом і містить ЛИШЕ
...
# балансувальників, адреси вузлів і шлях до файлу доступу разом з іменем користувача.
...
# Свідчення потрібні, щоб звіт можна було комусь показати і він щось значив.
evidence() {
  _evidence+=("### $1")
  _evidence+=('```')
  _evidence+=("$2")
...
need_kubeconfig() {
  if [ -z "${KUBECONFIG:-}" ]; then
    fail "не задана змінна KUBECONFIG" \
...
need_tenant() {
  if [ -z "${COZY_TENANT:-}" ]; then
    printf '%s[ FAIL ]%s не задана змінна COZY_TENANT\n' "$_C_FAIL" "$_C_OFF"
...
# Куди складаються машиночитані результати. Поза репозиторієм навмисно: всередині
...
_write_result_json() {
  ...
  # прогонів на одному кластері і різний у різних людей, а головне — його не можна
...
finish() {
  ...
  printf '\n'
  printf 'перевірок: %d · пройшло: %d · провалено: %d · попереджень: %d\n' \
...

  {
    ...
    echo "- Перевірок: ${total} (пройшло ${_pass}, провалено ${_fail}, попереджень ${_warn})"
...
# Людиночитаний розмір: Kubernetes віддає allocatable то в Ki, то в голих байтах,
# і «3258002390» у звіті читачеві нічого не говорить.
human_bytes() {
...
#
# Навіщо так. Усе, що потрапляє в args Pod, видно будь-кому, хто має `get pods`,
...
# скриптом, який робить саме це, було б подвійним стандартом.
#
# Використання:
...
in_cluster_with_secrets() {
  ...
  # Secret створюється зі stdin, тому значення не потрапляють в аргументи kubectl.
  local args=()
...
# а без неї скрипти перевірки не працюють у строгих кластерах.
# Аргументи команди передаються КОЖЕН ОКРЕМО, а JSON збирається пітоном:
# ручне екранування лапок у bash уже призводило до битого override і мовчазної
...
# Виконати команду в одноразовому Pod і повернути її вивід.
...
in_cluster_curl() {
  ...
  kubectl run "$name" --rm -i --restart=Never --quiet \
    ...
    2>/dev/null
  local rc=$?
  # `--rm` видаляє Pod, лише поки клієнт приаттачений: обрив, таймаут чи Ctrl+C
  # залишають його висіти. Явне видалення — щоб скрипт не смітив у кластері.
  kubectl delete pod "$name" --ignore-not-found --wait=false >/dev/null 2>&1
  return $rc
}

# Зібрати відповіді від КІЛЬКОХ запитів поспіль, по одному на рядок.
#
