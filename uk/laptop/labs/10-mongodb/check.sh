...
# побудовано розріджений індекс, валідатор схеми увімкнено, а документів без типу
# не залишилося.
#
...
# Усе, що далі, потребує входу до бази. Без пароля скрипт не гадає і не мовчить,
...
# Пароль процентно кодується: символи @ : / ? # % у ньому інакше руйнують рядок
...
# він несумісний із багаторядковим --eval без переускладнення. Pod живе секунди і
...
# `--overrides` із securityContext: без нього Pod не створиться в кластері з профілем
# `restricted`, і лаба провалиться з причини, що до учасника стосунку не має.
...
# через python, який сам його візьме в лапки, а імена бази і колекції підставляються
# за мітками нижче.
...
# Відмову в автентифікації відділено від інших помилок: у неї своя часта причина —
  ...
  case "$ERR" in
    *[Aa]uthentication*)
      fail "MongoDB не прийняла реквізити користувача ${MONGO_USER}" \
           "перевірте пароль і те, що в рядку підключення є authSource=admin: користувач заведений у базі admin, а права видані в ${MONGO_DB}" ;;
    *)
...
if [ "$TOTAL" -ge 4 ]; then
  ...
  fail "у колекції ${MONGO_COLL} усього ${TOTAL} документів, очікувалося не менше чотирьох" \
...
if [ "$TYPES" -ge 4 ]; then
  ...
  fail "різних типів перепустки всього ${TYPES}, очікувалося чотири" \
...
# --- 5. валідатор схеми увімкнено ------------------------------------------
VALIDATOR="$(num validator)"
ACTION="$(mget validationAction)"
if [ "$VALIDATOR" = "1" ]; then
  ok "валідатор схеми увімкнено (дія при порушенні: ${ACTION:-за замовчуванням})"
  if [ "$ACTION" = "warn" ]; then
    warn "валідатор лише попереджає, але документи приймає" \
         "для бойової колекції потрібен validationAction: error"
  fi
else
  fail "валідатор схеми не увімкнено — опечатка в імені поля пройде мовчки" \
...
if [ "$TYPELESS" -eq 0 ]; then
  ok "документів без поля type не залишилося"
else
