#!/usr/bin/env bash
# Перевірка лаби 9: у ClickHouse лежить журнал проходів і за ним рахується звіт.
#
# Перевіряємо не «сервіс створено», а суть: таблиця є, рядків не менше мільйона,
# дані різноманітні й з вираженими піками, звіт за місяцями відпрацьовує за
# мілісекунди, а запит за однією колонкою читає малу частку таблиці — тобто
# колонковість працює, а не заявлена.
#
...
#
# Пароль не друкується і у звіт не потрапляє.
...
# Адреса внутрішня, із самого кластера: 8123 — порт HTTP-інтерфейсу ClickHouse.
...
# відокремлює «немає зв'язку» від «зв'язок є, пароль не той».
...
if printf '%s' "$PING" | grep -qi 'ok'; then
  ...
  fail "ClickHouse не відповідає за адресою ${CH_HOST}" \
       "перевірте номер тенанта в COZY_TENANT і ім'я застосунку (за замовчуванням 'analytics'; інакше CH_APP=ім'я ./check.sh); у дашборді застосунок має бути в готовому стані"
  finish
  exit $?
fi
...
if [ "$PEAK_MIN" -gt 0 ] && [ "$PEAK_MAX" -ge $((PEAK_MIN * 2)) ]; then
  ok "у даних є виражені піки за годинами (найнавантаженіша година до найтихішої — не менш ніж удвічі)"
  evidence "Розподіл за годинами" "максимум за годину: ${PEAK_MAX}
мінімум за годину: ${PEAK_MIN}"
else
  warn "піків за годинами не видно: максимум ${PEAK_MAX}, мінімум ${PEAK_MIN}" \
       "звіт «коли піки» на таких даних беззмістовний; перевірте, що генератор відпрацював цілком"
fi

# --- 4. звіт за місяцями рахується швидко -----------------------------------
REPORT="$(ch_query <<SQL
...
if [ -z "$ELAPSED" ]; then
  fail "звіт за місяцями не відпрацював" \
  ...
  # Поріг тримаємо близько до того, що обіцяє лаба. Колишні п'ять секунд зараховували
  ...
  if [ "$FAST" = "1" ]; then
    ok "звіт за місяцями пораховано за ${MS} мс, прочитано рядків: ${READ_ROWS}"
  elif [ "$SLOW" = "1" ]; then
    fail "звіт за місяцями рахувався ${MS} мс — це не той порядок, про який лаба" \
    ...
    warn "звіт за місяцями пораховано за ${MS} мс — повільніше за очікуване, але в межах розумного" \
         "у зайнятому тестовому середовищі так буває; у вільному такий звіт вкладається в десятки мілісекунд"
  fi
  evidence "Звіт за місяцями" "час: ${MS} мс
прочитано рядків: ${READ_ROWS}"
fi

# --- 5. колонковість працює, а не заявлена ----------------------------------
# Запит торкається однієї маленької колонки. Якщо сховище колонкове, прочитано
...
# Обидві величини НЕСТИСНУТІ: `bytes_read` у статистиці запиту — це розпакований
...
if [ "$NARROW_BYTES" -gt 0 ] && [ "$TABLE_BYTES" -gt 0 ]; then
  ...
  evidence "Читання однієї колонки" "прочитано байтів: ${NARROW_BYTES}
уся таблиця без стиснення, байтів: ${TABLE_BYTES}
частка: ${SHARE}%"
  ...
  if [ "$SHARE" -le 25 ]; then
    ok "запит за однією колонкою прочитав ${SHARE}% даних таблиці — колонкове зберігання працює"
  elif [ "$NARROW_BYTES" -lt "$TABLE_BYTES" ]; then
    warn "запит за однією колонкою прочитав ${SHARE}% даних таблиці — менше цілого, але виграш скромніший за очікуваний" \
         "очікувались одиниці відсотків; перевірте, що запит звертається до однієї вузької колонки, а не до кількох"
  else
    warn "запит за однією колонкою прочитав не менше всієї таблиці" \
