#!/usr/bin/env bash
# Перевірка лаби 5: стан кластера надходить із Git і утримується звіркою.
#
# Запускається на вашому кластері `lab`, із теки лаби, вами ж:
...
# призупинений Flux — це найчастіший спосіб пройти лабу повз її сенс.

LAB_NAME="05-gitops"
LAB_TITLE="Лаба 5 · Інфраструктура в Git"
# Спільна обв'язка всіх лаб: із неї беруться ok / fail / warn / evidence / finish і
# перевірки оточення. Шлях обчислюється від розташування цього файлу, тому скрипт
# можна запускати з будь-якої теки.
. "$(cd "$(dirname "$0")/../../check" && pwd)/lib.sh"

# Без файлу доступу до кластера перевіряти нічого — виходимо одразу і зі зрозумілою причиною.
need_kubeconfig

# Імена, які лаба створює. Зібрані в одному місці: якщо учасник назвав об'єкти
# інакше, правити треба тут, а не шукати імена по всьому скрипту.
NS_APP="passes"
GITREPO="passes"
KUSTOMIZATION="passes"

# Читаємо поле об'єкта, не падаючи, якщо об'єкта або CRD немає.
...
# kustomize-controller застосовує завантажене. Без другої нічого не потрапить у кластер.
...
# Три різні наслідки, і плутати їх не можна: об'єкта немає зовсім; об'єкт є, але в ньому
# залишилася заглушка адреси; об'єкт є і адреса справжня, але Flux не зміг прочитати
...
if ! kubectl api-resources --api-group=source.toolkit.fluxcd.io 2>/dev/null | grep -q gitrepositories; then
  ...
  if [ -z "$GR_URL" ]; then
    ...
    fail "у GitRepository залишилася адреса-заглушка" \
...
revision: ${GR_REV:-невідома}"
    ...
    fail "Flux не може прочитати репозиторій ${GR_URL}" \
         "дивіться flux get sources git; найчастіше це помилка в адресі, приватний репозиторій або інша гілка"
...
if ! kubectl api-resources --api-group=kustomize.toolkit.fluxcd.io 2>/dev/null | grep -q kustomizations; then
  ...
  # спосіб «скласти» лабу, не отримавши жодної її вигоди.
  if [ "$KS_SUSPEND" = "true" ]; then
    ...
    ok "звірка активна: розходження з Git буде усунуто саме, інтервал ${KS_INTERVAL:-за замовчуванням}"
  fi
  ...
  if [ "$KS_PRUNE" = "true" ]; then
    ...
    warn "prune вимкнено — видалене з репозиторію залишиться працювати в кластері" \
         "поставте prune: true у flux/kustomization.yaml, інакше Git описує стан лише наполовину"
  fi
fi

# --- об'єкти в кластері належать Flux, а не були застосовані руками ---------
...
if [ -z "$(kget deployment passes -n "$NS_APP" -o name)" ]; then
  ...
  fail "застосунок passes є, але його створив не Flux" \
       "приберіть його (kubectl delete ns ${NS_APP}) і дайте Flux розгорнути його з Git заново"
fi

# --- застосунок дійсно відповідає --------------------------------------
# Об'єкт у кластері і працюючий сервіс — різні речі: Deployment може бути створений,
# а Pod падати в циклі. Тому йдемо всередину кластера і запитуємо сервіс за його
...
# до попередньої версії, але Git про це не дізнається, і наступна ж звірка поверне погану
...
REPO="${LAB_REPO:-}"
if [ -z "$REPO" ]; then
  warn "історію репозиторію не перевірено: не задано змінну LAB_REPO" \
       "щоб перевірити і її: export LAB_REPO=~/passes-gitops && ./check.sh"
elif [ ! -d "$REPO/.git" ]; then
  warn "у ${REPO} немає клону репозиторію" \
       "вкажіть теку, у яку ви робили git clone"
else
  ...
  if [ -n "$HEAD_SHA" ] && printf '%s' "${KS_REV:-}" | grep -q "$HEAD_SHA"; then
    ...
    warn "коміт у кластері (${KS_REV:-невідомий}) відрізняється від локального HEAD (${HEAD_SHA})" \
         "перевірте, що локальні коміти надіслано (git push), і зачекайте на інтервал звірки"
  fi
fi
