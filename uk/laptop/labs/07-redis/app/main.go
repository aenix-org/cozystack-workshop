...
//	            спершу дивиться в кеш.
//
// Одна роль на обидва випадки, бо образ має бути один: два майже однакові
// образи в реєстрі — це два місця, де можна забути оновити версію.
//
// Зовнішніх залежностей немає, лише стандартна бібліотека. Клієнт Redis тут
...
//
// Читати на Go вміти не потрібно: нижче розмічено, де що лежить. Спершу дрібні помічники,
...
//
// Збирається файл сусіднім Dockerfile, на ноутбуці: docker build ... app/ — див. README.
package main

// Список бібліотек, якими користується файл. Усі до одної — стандартні, з постачання Go.
...
// env читає змінну оточення і, якщо та порожня чи не задана, повертає запасне
...
// падає: пише в лог і бере запасне значення. Друкарська помилка в маніфесті не повинна класти
...
// від невірного пароля не рятує, а спроби лише маскують причину.
...
// connectLocked відкриває з'єднання і, якщо пароль задано, тут же представляється
// командою AUTH. Суфікс Locked в імені означає «викликати лише тоді, коли замок mu
// вже взято» — це домовленість між цими функціями, а не властивість мови.
...
// Спроб рівно дві, не десять: якщо Redis відповідає відмовою, повтори лише затримають
...
// commandLocked відправляє команду в тому вигляді, в якому її розуміє Redis: спершу
...
// задають імена полів у JSON: у Go поля видно ззовні лише з великої літери, а в JSON
...
// інакше з відповіді не можна було б зрозуміти, кеш це чи похід у довідник.
...
// на нього /healthz і, дивлячись на MODE, одну з двох ролей. Роль обирається один раз
...
func main() {
	...
	case "api":
		setupAPI(mux, pod)
	default:
		log.Fatalf("невідомий MODE=%q, допустимі hr і api", mode)
	}
...
func setupAPI(mux *http.ServeMux, pod string) {
	...
	// Корінь «/» — візитка сервісу: версія, Pod, вузол, реєстр і режим кешу. За полем cache
	...
	mux.HandleFunc("/employee", func(w http.ResponseWriter, r *http.Request) {
		...
		// записів не зіткнулися: кеш один на все застосування, а імена в ньому плоскі.
		key := "employee:" + id
		...
		if cache != nil {
			...
			case err != nil:
				...
				log.Printf("кеш недоступний (%v), іду в довідник", err)
			case found:
				if json.Unmarshal([]byte(raw), &emp) == nil {
					fromCache = true
				} else {
					log.Printf("в кеші за ключем %s лежить сміття, іду в довідник", key)
				}
			}
		}
		...
		// Поля cached і took_ms — те, заради чого все затівалося: за ними видно, чи потрапив запит
		// у кеш і у скільки мілісекунд це обійшлося. Їх же читає check.sh,
