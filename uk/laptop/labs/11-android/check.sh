...
#
# Перевіряємо не «Job створено», а три різні твердження, і вони не дорівнюють одне одному:
#   1) Job завершився успішно,
#   2) усередині нього справді зібрався APK (BUILD SUCCESSFUL),
...
#
# Скрипт нічого не змінює в кластері — лише читає та надсилає HTTP-запити.
...
# Ці дві змінні підхоплює lib.sh — вони потрапляють у заголовок звіту та в ім'я
...
# запит зсередини кластера та запис звіту. Шлях відраховується від місця, де лежить сам
...
# Значення ключа секрета. base64 -d не скрізь однаковий (BSD проти GNU),
...
# Перевіряємо не існування секрета, а те, що в ньому заповнені всі чотири поля.
...
if kubectl get secret "$SECRET" >/dev/null 2>&1; then
  ...
  if [ -z "$MISSING" ]; then
    ...
    # Значення ключів у звіт не потрапляють — лише імена полів.
...
secretKey: <приховано>"
  ...
  fail "у кластері немає секрета ${SECRET}" \
...
# Запит іде з Pod, а не з ноутбука: у ноутбука своя мережа та свої маршрути,
...
if [ -n "$EP" ]; then
  ...
  # зобов'язана падати там само, де впаде Job, а не видавати зелений на протухлому серті.
...
# Стан Pod розбирається окремо, бо «ще йде» і «висить у Pending» для
...
завершено: ${DURATION:-невідомо}"
    ...
    fail "Job ${JOB} завершився з помилкою (невдалих спроб: ${FAILED})" \
         "дивіться останні рядки логу: kubectl logs job/${JOB} --tail=40"
    evidence "Хвіст логу впалої збірки" \
  ...
  # лише після копіювання файлу в бакет. Друге сильніше за перше: APK може зібратися
  ...
  if [ -z "$LOGS" ]; then
    ...
    if printf '%s' "$LOGS" | grep -q 'BUILD SUCCESSFUL'; then
      ...
      fail "у логах немає рядка BUILD SUCCESSFUL — компіляція не дійшла до кінця" \
           "шукайте перший рядок із FAILURE: kubectl logs job/${JOB} | grep -n -m1 -A20 FAILURE"
    fi
    ...
    if [ -n "$UPLOADED" ]; then
      ...
      fail "APK зібрався, але в бакет не поїхав" \
           "дивіться хвіст логу: kubectl logs job/${JOB} --tail=20; найчастіше винен bucketName — у ньому потрібне довге ім'я з дашборда, а не 'builds'"
    fi
  fi
fi
  ...
  case "$BIGGEST_H" in
    *Gi)
      ...
      if [ "${GB_INT:-0}" -ge 6 ] 2>/dev/null; then
        ...
        warn "найбільший вузол віддає лише ${BIGGEST_H} пам'яті" \
             "збірка просить 4Gi лише під requests; якщо Job висить у Pending, збільшіть тип вузла до u1.large — як, написано в README"
      fi
      ;;
    *)
