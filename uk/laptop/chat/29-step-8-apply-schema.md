...
password: Orders2019!
```
Пароль заданий у `manifests/04-managed.yaml`, шукати його більше ніде не треба.
...
Друга і третя команди виглядають надлишково, але без них встановлення падає, і обидві помилки
...
- без `--disablerepo`/`--enablerepo` — `HTTPS Error 410 - Gone`. Пакет репозиторію
  вмикає разом усі версії PostgreSQL, зокрема зняті з підтримки 12-ту та 13-ту, а `yum`
  перед встановленням обходить **кожен** увімкнений репозиторій і падає на першому мертвому.
  Ми явно лишаємо тільки той, який нам потрібен.
...
Якщо відповідь — `command not found`, клієнт покладено повз `PATH`; знайдіть його і допишіть
...
- `-h postgres-db-rw.tenant-workshopXX.svc.cozy.local` — адреса бази. Це **не IP**,
  а внутрішнє ім'я в кластері. Суфікс `-rw` важливий: у керованого Postgres кілька копій,
  і це ім'я завжди вказує на ту, у яку **можна писати**. Є парне ім'я з `-ro`
  — тільки для читання. Під час перемикання ролей між копіями ім'я не змінюється, тому в
  налаштуваннях застосунку прописують саме його, а не адресу конкретного сервера.
- `-U orders` — під яким користувачем, `-d orders` — у яку базу.
- `-f orders-schema.sql` — виконати команди з файлу.

Саме можливість звертатися до бази за постійним ім'ям, а не за IP, і робить
