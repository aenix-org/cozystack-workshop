...
#   ./bundle.sh --rerun 07-redis  перепройти одну лабу й оновити її результат
#
set -uo pipefail
...
Пройдіть хоча б одну й запустіть перевірку — потім повертайтеся сюди.

Якщо ви проходили лаби на іншій машині, скопіюйте звідти папку
...
# Те саме звичайним текстом — щоб людина бачила, що відправляє.
...
with open(txt, "w") as fh:
    ...
    fh.write("\nКластерів: %d. Версії Kubernetes: %s\n"
             % (len(uids), ", ".join(sorted(versions)) or "не визначено"))
...
if len(uids) > 1:
    print("\nУвага: результати зняті з %d різних кластерів. Це припустимо, "
          "але при завантаженні набір буде позначено." % len(uids))
for p in problems:
    print("  проблема: %s" % p)
print("\nФайл для завантаження: %s" % out_path)
print("Він же звичайним текстом:  %s" % txt)
print("Перегляньте текстовий файл перед відправкою — у ньому видно все, що йде.")
PYEOF
